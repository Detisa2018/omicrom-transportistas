    ...
    <div id="header">
        <table style="width: 100%;border-collapse: collapse; border: 0px solid white;" aria-hidden="true">
            <tr style="background-image: url('libnvo/fondo_top_verde.png');height: 120px;">
                ...
                <td style="min-width: 200px;vertical-align: middle;text-align: right;padding-right: 25px;">
                    <?php if ($lBd) { ?>
                        <p style="text-align: right" class='nombre_cliente'><br><br><?= $cFecha ?> &nbsp; &nbsp;</p>
                    <?php } ?>
                    <div id='hora' style="text-align: right"></div>
                    <?php if ($ciaVO->getTipo_permiso() === "TRA") { ?>
                        <div style="color: #066;
                             font-size: 15px;
                             font-weight: bold;
                ...
                <td class="tdContenido" valign="top">
                    ...
                    <table style="width: 100%;border-collapse: collapse;" aria-hidden="true">
                        <tr>
                            ...
                            <?php if ($usuarioSesion->getLevel() > 5 && ($CiaVO->getTipo_permiso() === "EXP" || $CiaVO->getTipo_permiso() === "COM")) { ?>
...
function PieDePagina() {
    ...
    <div id="footer">
        <table aria-hidden="true">
            <tr>
                <td>
                    DETISA ::: Normal de Maestros No.10 Col. Tulantongo Texcoco Estado de Mexico Cp. 56217 Tel. 595 92 50401 ó 55 6255 3855 www.detisa.com.mx
                </td>
...
function TotalizaCorte() {
    ...
    echo "<tr>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dClientes' data-clientes='" . number_format($nCredito + $nAceCre, 2, ",", "") . "'>$ " . number_format($nCredito + $nAceCre, 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dTarjetas' data-tarjetas='" . number_format($nTarjeta + $nAceTar, 2, ",", "") . "'>" . number_format($nTarjeta + $nAceTar, 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dConsignaciones' data-consignaciones='" . number_format($nConsignacion + $nAceConsig, 2, ",", "") . "'>" . number_format($nConsignacion + $nAceConsig, 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dMonederos' data-monederos='" . number_format($nMonedero + $nAceMon, 2, ",", "") . "'>" . number_format($nMonedero + $nAceMon, 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dAceites' title='La venta de contado de aceites ya esta incluido en el efectivo total.' data-aceites='" . number_format($nAceTotal, 2, ",", "") . "'><mark>" . number_format($nAceTotal, 2) . "<mark></td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dDolares' data-dolares='" . number_format($Dol[1], 2, ",", "") . "'>" . number_format($Dol[1], 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dGastos' data-gastos='" . number_format($nGastos, 2, ",", "") . "'>" . number_format($nGastos, 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dEfectivo' data-efectivo='" . number_format($nEfectivoDisplay, 2, ",", "") . "'>" . number_format($nEfectivoDisplay, 2) . "</td>";
    echo "<td align='right' bgcolor='#f1f1f1' id='dTotal' data-total='" . number_format($TotalVtaDisplay + $nAceTotal, 2, ",", "") . "'><strong>" . number_format($TotalVtaDisplay + $nAceTotal, 2) . "</strong></td>";
    echo "<td bgcolor='#f1f1f1'>vs</td>";
    if ($usuarioSesion->getTeam() == 'Administrador' OR $usuarioSesion->getTeam() == 'Supervisor' OR $banderaBanc == TRUE) {
        echo "<td align='right' bgcolor='#f1f1f1' id='dDepositos' data-depositos='" . number_format($nBancos, 2, ",", "") . "'>" . number_format($nBancos, 2) . "</td>";
    }
    echo "</tr></table></div><input type='hidden' name='IdCt' id='IdCt' value='$Corte'><input type='hidden' name='IdUsr' id='IdUsr' value='" . $usuarioSesion->getId() . "'>";
...
function generaDescargaXmlsByVenta($parameters, $NombreZip) {
    try {
        ...
        $zipFile = new ZipArchive();
        //$filename = "/tmp/" . date("Ymd_His") . ".zip";
        $filename = "/home/omicrom/xml/" . date("Ymd_His") . ".zip";
        ...
        $opened = $zipFile->open($filename, ZipArchive::CREATE | ZipArchive::OVERWRITE);
        error_log('FileName: ' . $opened);
        if ($opened === TRUE):
            error_log("Llenando Datos: ");
            //$tidy = new tidy();
            error_log("Se creo el objeto ");
            $rows = utils\IConnection::getRowsFromQuery($selectFacturas);
            error_log("Find rows: " . count($rows));
            foreach ($rows as $rg):
                ...
                $zipFile->addFromString($name_xml, $rg["cfdi_xml"]);
            endforeach;
            $zipFile->close();
...
function getYears() {
    $mysqli = iconnect();
    $years = array();
    $TipoProveedor = "SELECT tipo_permiso FROM cia";
    $Tp = utils\IConnection::execSql($TipoProveedor);
    switch ($Tp["tipo_permiso"]) {
        case"EXP":
            $selectYears = "SELECT YEAR(fin_venta) year FROM rm GROUP BY YEAR(fin_venta);";
            break;
        case"TRA":
            $selectYears = "SELECT year FROM "
                    . "(SELECT YEAR(fecha) year FROM traslados GROUP BY YEAR(fecha) "
                    . "UNION ALL "
                    . "SELECT YEAR(fecha) year FROM ingresos GROUP BY YEAR(fecha)) v "
