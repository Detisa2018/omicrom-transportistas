...
//                $rs["factoriva"] > 0 ? $concepto->setObjetoImp("02") : $concepto->setObjetoImp("01");
                ...
                if ($rs['factorivaretenido'] > 0) {
                    ...
                    $InsertAbonoPorRetencion = "INSERT INTO cxc (cliente,placas,referencia,fecha,hora,tm,concepto,cantidad,importe,recibo,corte,producto,rubro,factura,fechaHoraRegistro) 
                                    SELECT cliente,'Retencion',id,'" . date("Y-m-d") . "','" . date("H:i:s") . "','H','Retencion de IVA','1', " . number_format($base1, 2, '.', '') . ",0,
                                    (SELECT id FROM ct WHERE status='Abierto' AND statusctv='Abierto'),'-','-----',id,now() FROM fc WHERE id =  " . $this->folio . ";";
                    if (($this->mysqlConnection->query($InsertAbonoPorRetencion))) {
                ...
                if (substr($rs["ClaveProdServ"], 0, 6) == "801315" && !empty($rs["cuenta_predial"])) {
                    $cuentaPredial = new cfdi40\Comprobante40\Conceptos\Concepto\CuentaPredial();
                    $cuentaPredial->setNumero($rs["cuenta_predial"]);
                    $concepto->addCuentaPredial($cuentaPredial);
                }
...
//cfdiRelacionados
    ...
    private function conceptosDescuento() {
        ...
        $sql = "SELECT "
                ...
                . "round(fcd.cantidad * round(preciou ,4) ,2) + IF(cli.desgloseIEPS = 'S',0.0000,round(fcd.cantidad * round(ROUND(fcd.factorieps,6),6) ,2)) Importe,"
                . "fcd.cantidad Cantidad, "
                //. "ROUND( ( fcd.subtotal + diferencia + ROUND( IF( cli.desgloseIEPS = 'S', 0.0000, fcd.tax_ieps ), 4 ) ) / ( fcd.preciou + ROUND( IF( cli.desgloseIEPS = 'S', 0.0000, fcd.factorieps ), 4 ) ), 4 ) Cantidad, "
                . "fcd.tax_iva, "
                . "IF( cli.desgloseIEPS = 'S', fcd.tax_ieps, 0.00 ) tax_ieps "
