...
//                $this->comprobante->setFormaPago($rs["formaPago"]);
        ...
        function cartaPorte() {
            ...
            $sql = "SELECT carta_porte.*,catalogo_vehiculos.permiso_sct,catalogo_vehiculos.numero_sct,catalogo_vehiculos.nombre_aseguradora,"
                    ...
                    . "catalogo_operadores.rfc_operador,catalogo_operadores.nombre,catalogo_operadores.num_licencia, catalogo_vehiculos.peso_bruto ,"
                    ...
                    . "WHERE catalogo_direcciones.tabla_origen IN ('D','P','C') AND cpd.origen='TRA'  AND carta_porte.id = (SELECT id FROM carta_porte WHERE tabla = 'Traslados' "
            ...
            if ($query = $this->conn->query($sql)) {
                if ($rs = $query->fetch_assoc()) {
                    ...
                    error_log("ENTRAMOS DESPUES DE NO ");
                    ...
                    error_log("ENTRAMOS Total Recorrido ");
                    ...
                    error_log("ENTRAMOS DESPUES CCP " . $CCP);
                    ...
                    WHERE id_carta_porte_fk = " . $rs["id"] . " AND cpd.tipo = 'Origen' AND cpd.origen = 'TRA' AND cd.tabla_origen='P';";
                    ...
                    if ($CartaP = $this->conn->query($CartaPorte)) {
                        ...
                        while ($cP = $CartaP->fetch_assoc()) {
                            ...
                            $NombreRfc = "SELECT cli.nombre,cli.rfc FROM omicrom.carta_porte cp "
                                    ...
                                    . "WHERE cp.tabla = 'Traslados' AND cp.id = " . $rs["id"] . "";

                            $NombreRfc = "SELECT cli.rfc, cli.nombre FROM carta_porte_destino cpd LEFT JOIN catalogo_direcciones cd 
                            ON cpd.id_destino_fk = cd.id LEFT JOIN cli ON cli.id=cd.id_origen 
                            WHERE id_carta_porte_fk = " . $rs["id"] . " AND cpd.tipo = 'Destino' AND cpd.origen = 'TRA'";
...
//                    }
                    ...
                    $Remolques = "SELECT * FROM omicrom.carta_porte_remolques where id_carta_porte_fk = (select id "
                            . "FROM carta_porte WHERE id_origen=" . $this->folio . " AND tabla ='Traslados');";
                    error_log($Remolques);
