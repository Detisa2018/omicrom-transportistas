...
class TerminalPosDAO implements FunctionsDAO {
    ...
    public function update($objectVO) {
        $Update0 = "UPDATE ctg_hardware SET serial = ?, model = ?, additional = ?, status = ?, device = ? WHERE id_device = ?";
        if (($ps = $this->conn->prepare($Update0))) {
        ...
        $Update1 = "UPDATE pos_data SET ip = ?  WHERE pos_id = ?";
        if (($ps = $this->conn->prepare($Update1))) {
            $ps->bind_param("si",
                    $objectVO->getIp(),
                    $objectVO->getId()
            );
