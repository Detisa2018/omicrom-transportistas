...
class TrasladosDAO implements FunctionsDAO {
    ...
    public function create($objectVO = TrasladosVO) {
        ...
        if (($ps = $this->conn->prepare($sql))) {
            ...
            if ($ps->execute()) {
                error_log("Creación de traslados");
                $id = $ps->insert_id;
                $ps->close();
                return $id;
            } else {
    ...
    public function retrieve($idObjectVO, $field = "id") {
        $objectVO = new TrasladosVO();
        $sql = "SELECT t.id,t.id_cli,t.serie,t.folio, t.claveProductoServicio,t.fecha,t.cantidad, t.importe,t.iva,t.ieps,t.total,t.status, t.id_prv,
    ...
    public function update($objectVO = TrasladosVO) {
        $sql = "UPDATE " . self::TABLA . " SET "
                ...
                . "WHERE id = ? ";
        if (($ps = $this->conn->prepare($sql))) {
            $ps->bind_param("issssssssssssssssssii",
                    $objectVO->getId_cli(),
                    $objectVO->getSerie(),
                    $objectVO->getFolio(),
                    $objectVO->getClaveProductoServicio(),
