...
class FcDAO implements FunctionsDAO {
    ...
    public function create($objectVO = FcVO) {
        $id = -1;
        $HtmlConcat = ($objectVO->getFecha() != "" || $objectVO->getFecha() != null) ? "'" . $objectVO->getFecha() . "'" : "NOW()";
        $sql = "INSERT INTO " . self::TABLA . " ("
                ...
                . "cancelacion"
                . ") "
                . "SELECT ?,IFNULL( ( SELECT MAX( folio ) FROM fc WHERE fc.serie =  ? ), 0 ) + 1, ?, $HtmlConcat, ?, ?, ?, ?, ?, ?, ?, '-----', ?,? LIMIT 1";
    ...
    public function update($objectVO = FcVO) {
        ...
        if (($ps = $this->conn->prepare($sql))) {
            ...
            error_log("ANTES DE ACTUALIZAR");
            if ($ps->execute()) {
                error_log("Execute " . print_r($ps, true));
                return true;
            }
        }
        error_log($this->conn->error);
    ...
    public function createDataAll($objectVO = FcVO) {
        $id = -1;
        $sql = "INSERT INTO " . self::TABLA . " ("
                . "serie, "
                . "folio, "
        ...
        //error_log($sql);
        if (($ps = $this->conn->prepare($sql))) {
            $ps->bind_param("sssssssssssssssss",
                    $objectVO->getSerie(),
                    $objectVO->getSerie(),
                    $objectVO->getCliente(),
                    $objectVO->getCantidad(),
...
abstract class SerieValidacionCorporativo extends BasicEnum {

    const SIN_PROCESAR = "Sin Procesar";    /* STATUS DE OMICROM */
    const VALIDO = "Valido";    /* STATUS DE OMICROM */
    const FACTURADO = "Facturado";  /* STATUS DE OMICROM */
    const LOCAL = "Local";  /* STATUS DE OMICROM */
    const APARTADO = "Apartado";    /* STATUS DE CORPORATIVO */
