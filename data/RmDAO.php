...
class RmDAO implements FunctionsDAO {
    ...
    public function create($objectVO = RmVO) {
        ...
        if (($ps = $this->conn->prepare($sql))) {
            ...
            $ps->bind_param("sssssssssssssssssssssssis",
...
function getResumenVentas($Connection, $TipoVenta, $FechaInicial, $FechaFinal, $Vol, $pesos, $Periodo) {
    ...
    $ICorteDia = "SELECT valor FROM variables_corporativo WHERE llave='IgualCorteDia'";
    if (($query = $Connection->query($ICorteDia))) {
    ...
    if ($object[0][0] == "1" && $Periodo === "Cortes") {
        $pr = "importe";
        $volx = "importe/rm.precio";
    }

    $TipoDesucentoVolumen = $Periodo === "Cortes" ? "if(rm.tipo_venta='N',rm.$Vol,rm.volumenp)" : "if(rm.tipo_venta='N',rm.$Vol,rm.volumenp)";
    $TipoDesucentoImporte = $Periodo === "Cortes" ? " ROUND(if(rm.tipo_venta='N',rm.$pesos,rm.$pr),2)" : " ROUND(if(rm.tipo_venta='N',rm.$pesos,rm.$pr),2)";
