...
namespace com\detisa\omicrom {
    ...
    require_once ('com/softcoatl/cfdi/complemento/cartaporte/CartaPorte31.php');
    ...
    class CartaPorteIngresoDAO {
        ...
        private function comprobante() {
            $sql = "SELECT id.preciob,(id.preciob/1.16) subt,id.iva,id.ieps,cp.moneda,DATE_FORMAT(i.fecha, '%Y-%m-%dT%H:%i:%s') fechat,i.metodopago metodoPago,
                        i.formadepago formaPago,i.usocfdi usoCfdi,i.folio,i.serie, IFNULL(i.observaciones,'-') observaciones FROM ingresos i LEFT JOIN ingresos_detalle id ON i.id = id.id 
            ...
            if (($query = $this->conn->query($sql)) && ($rs = $query->fetch_assoc())) {
                ...
                $Observacion = new Adenda\detisa\Observaciones\Observacion($rs["observaciones"]);
        ...
        private function conceptos() {
            ...
            $Sql2 = "SELECT ClaveProdServ claveProductoServicio,nombre,id_servicios_tra FROM ingresos t LEFT JOIN cfdi33_c_conceptos cc ON "
                    . "t.ClaveProdServ = cc.clave where id =" . $this->folio . "";
            if ($query0 = $this->conn->query($Sql2)) {
                $rs0 = $query0->fetch_assoc();
            }
            $ConceptosDefinidos = "SELECT nombre,clave_unidad,clave_producto,identificador FROM omicrom.servicios_tra WHERE id = " . $rs0["id_servicios_tra"];
            $DataDt = \com\softcoatl\utils\IConnection::execSql($ConceptosDefinidos);

            $SqlDesc = "SELECT descripcion FROM omicrom.ingresos_detalle id LEFT JOIN inv ON inv.id=id.producto "
                    . "WHERE id.producto = 0 AND id.id =" . $this->folio . "";
            if ($qDesc = $this->conn->query($SqlDesc)) {
                $Ds = $qDesc->fetch_assoc();
            }
            $total = 0.00;
            ...
            if (($query = $this->conn->query($sql))) {
                ...
                while (($rs = $query->fetch_assoc())) {
                    ...
                    $concepto->setClaveProdServ($DataDt["clave_producto"]);
                    $concepto->setClaveUnidad($DataDt["clave_unidad"]);
                    $concepto->setDescripcion($DataDt['nombre']);
                    $concepto->setImporte(number_format($rs['preciob'] / (1 + $rs["iva"]), 2, '.', ''));
                    $concepto->setCantidad(number_format($rs['cantidad'], 2, '.', ''));
                    $concepto->setNoIdentificacion($DataDt["identificador"] . $rs['id']);
...
//$this->comprobante->setSubTotal(number_format($subTotal, 2, '.', ''));
        ...
        function cartaPorte() {
            $cartaPorte = new complementos\cartaporte\CartaPorte31();
            $Ubicaciones = new complementos\cartaporte\CartaPorte31\Ubicaciones();
            $cartaPorteU = new complementos\cartaporte\CartaPorte31\Ubicaciones\Ubicacion();
            ...
            $sql = "SELECT carta_porte.*,catalogo_vehiculos.permiso_sct,catalogo_vehiculos.numero_sct,catalogo_vehiculos.nombre_aseguradora,"
                    ...
                    . "catalogo_operadores.rfc_operador,catalogo_operadores.nombre,catalogo_operadores.num_licencia,catalogo_operadores.registro_fiscal ,catalogo_operadores.recidencia_fiscal ,"
                    . "catalogo_direcciones.colonia coloniaD, catalogo_direcciones.localidad localidadD, catalogo_direcciones.municipio municipioD, catalogo_vehiculos.peso_bruto ,"
                    ...
                    . "WHERE catalogo_direcciones.tabla_origen in ('D','P','C') AND carta_porte.id = (SELECT id FROM carta_porte WHERE tabla='Ingresos' "
                    . "AND id_origen = " . $this->folio . ") LIMIT 1;";
            ...
            if ($query = $this->conn->query($sql)) {
                if ($rs = $query->fetch_assoc()) {
                    ...
                    // Versión 4 (aleatoria) según RFC 4122
                    $data[6] = chr(ord($data[6]) & 0x0f | 0x40);
                    $data[8] = chr(ord($data[8]) & 0x3f | 0x80);
                    $CCP = substr_replace(vsprintf('%s%s-%s-%s-%s-%s%s%s', str_split(bin2hex($data), 4)), 'CCC', 0, 3);
                    $cartaPorte->setIdCCP($CCP);
                    /* Ubicacion Origen */
                    ...
                    if ($RsOrg = $this->conn->query($Origen)) {
                        ...
                        $rfcOrigen = "SELECT prv.rfc, prv.nombre FROM carta_porte_destino cpd LEFT JOIN catalogo_direcciones cd 
                    ON cpd.id_destino_fk = cd.id LEFT JOIN prv ON prv.id=cd.id_origen 
                    WHERE id_carta_porte_fk = " . $rs["id"] . " AND cpd.tipo = 'Origen' AND cpd.origen = 'ING' AND cd.tabla_origen='P';";
                        $rfcPrv = $this->conn->query($rfcOrigen);
                        $prvrfc = $rfcPrv->fetch_assoc();
                        $cartaPorteU->setRfcRemitenteDestinatario($prvrfc["rfc"]);
                        $cartaPorteU->setNombreRemitenteDestinatario($prvrfc["nombre"]);
                        $cartaPorteU->setFechaHoraSalidaLlegada($rs["fechaHoraSalidaLlegada"] . ":01");
                        $cartaPorteDom = new complementos\cartaporte\CartaPorte31\Ubicaciones\Ubicacion\Domicilio();
                        ...
                        $cartaPorteDom->setLocalidad($rscia["localidad"]);
...
//                        $cartaPorteDom->setEstado($rscia["estadoClv"]);
                    ...
                    if ($CartaP = $this->conn->query($CartaPorte)) {
                        ...
                        while ($cP = $CartaP->fetch_assoc()) {
                            $cartaPorteUD = new complementos\cartaporte\CartaPorte31\Ubicaciones\Ubicacion();
                            $cartaPorteDomD = new complementos\cartaporte\CartaPorte31\Ubicaciones\Ubicacion\Domicilio();
...
//                            $cartaPorteDomD->setColonia($cP["coloniacd"]);
                            ...
                            $NombreRfc = "SELECT cli.nombre,cli.rfc FROM omicrom.carta_porte cp "
                                    . "LEFT JOIN ingresos i ON i.id=cp.id_origen "
                                    . "LEFT JOIN cli ON cli.id = i.id_cli "
                                    . "WHERE cp.origen = 'CPI' AND cp.id = " . $rs["id"] . "";
                            $nomRfc = $this->conn->query($NombreRfc);
                            $dataCli = $nomRfc->fetch_assoc();
                            $cartaPorteUD->setRfcRemitenteDestinatario($dataCli["rfc"]);
                            $cartaPorteUD->setNombreRemitenteDestinatario($dataCli["nombre"]);
                    ...
                    /* Mercancia */
                    $cartaPorteMercancias = new complementos\cartaporte\CartaPorte31\Mercancias();

                    $cartaPorteMercancias->setUnidadPeso("KGM");
                    ...
                    while ($fcd = $rFacturad->fetch_assoc()) {
                        $cartaPorteMercancia = new complementos\cartaporte\CartaPorte31\Mercancias\Mercancia();
                        $cartaPorteMercanciaCT = new complementos\cartaporte\CartaPorte31\Mercancias\Mercancia\CantidadTransporta();
                        $cartaPorteMercanciaDM = new complementos\cartaporte\CartaPorte31\Mercancias\Mercancia\DetalleMercancia();

                        $cartaPorteMercancia->setMaterialPeligroso("Sí");
                        $ClaveMaterial = ($fcd["inv_cproducto"] === "15101515" || $fcd["inv_cproducto"] === "15101514") ? "1203" : "1202";
                        $cartaPorteMercancia->setCveMaterialPeligroso($ClaveMaterial);
...
//                        $cartaPorteMercancia->setDescripEmbalaje($enbalajeDesc["descripcion"]);
                    ...
                    $autoTransporte = new complementos\cartaporte\CartaPorte31\Mercancias\Autotransporte();
                    ...
                    $autoTransporteVehicular = new complementos\cartaporte\CartaPorte31\Mercancias\Autotransporte\IdentificacionVehicular();
                    $autoTransporteVehicular->setConfigVehicular($rs["conf_vehicular"]);
                    $autoTransporteVehicular->setPesoBrutoVehicular($rs["peso_bruto"]);
                    ...
                    /* Seguros */
                    $autoTransporteSeguros = new complementos\cartaporte\CartaPorte31\Mercancias\Autotransporte\Seguros();
...
//                    }
                    ...
                    $autoTransporteRemolques = new complementos\cartaporte\CartaPorte31\Mercancias\Autotransporte\Remolques();
                    ...
                    if ($rRem = $this->conn->query($Remolques)) {
                        ...
                        while ($rmRs = $rRem->fetch_assoc()) {
                            $autoTransporteRemolquesRem = new complementos\cartaporte\CartaPorte31\Mercancias\Autotransporte\Remolques\Remolque();
...
//            $autoTransporte->setRemolques($autoTransporteRemolques);
                    ...
                    /* Operador */
                    $FiguraTransporte = new complementos\cartaporte\CartaPorte31\FiguraTransporte();
                    $TiposFigura = new complementos\cartaporte\CartaPorte31\FigurasTransporte\TiposFigura();
                    $TiposFiguraDomicilio = new complementos\cartaporte\CartaPorte31\FigurasTransporte\TiposFigura\Domicilio();
                    $TiposFiguraPartesT = new complementos\cartaporte\CartaPorte31\FigurasTransporte\TiposFigura\PartesTransporte();
                    ...
                    if ($rs["recidencia_fiscal"] !== "MEX") {
                        $TiposFigura->setNumRegIdTribFigura($rs["registro_fiscal"]);
                        $TiposFigura->setResidenciaFiscalFigura($rs["recidencia_fiscal"]);
                    } else {
                        $TiposFigura->setRFCFigura($rs["rfc_operador"]);
                    }
