...
class BonificacionDAO {
    ...
    function calculaBonificacion($CntPuntos, $Ticket) {
        $Calculo = "SELECT importe,pp.valor_punto,ROUND(importe/pp.valor_punto,0) cantidad_puntos,"
                . "descuento,importe - ((rm.importe/valor_punto) * pp.valor_punto) total,((rm.importe/valor_punto) * pp.valor_punto) descuento "
                . "FROM rm LEFT JOIN periodo_puntos pp ON TRUE WHERE rm.id= $Ticket AND pp.activo = 1;";
        $rsC = utils\IConnection::execSql($Calculo);
        $Html = "<table style='width:100%;margin-top:15px;margin-bottom:15px;background-color:#D5D8DC;border-radius:5px;' class='texto_tablas'>"
                . "<tr><th>Id</th><th>Puntos</th><th>Importe </th><th>Descuento</th><th>Total</th></tr>";
        $Html .= "<tr><td>" . $Ticket . "</td><td>" . $rsC["cantidad_puntos"] . "</td><td>" . number_format($rsC["importe"], 2)
                . "</td><td>" . number_format($rsC["descuento"], 2) . "</td><td>" . number_format($rsC["total"], 2) . "</td></tr></table><input type='hidden' name='PuntosNecesarios' id='PuntosNecesarios' value='" . $rsC["cantidad_puntos"] . "'>";
        return $Html;
    }
    ...
    function ingresaBonificacion($CntPuntos, $IdUnidad, $Ticket) {
        $SelectIdUnidad = "SELECT codigo FROM unidades WHERE id = $IdUnidad";
        $IdU = utils\IConnection::execSql($SelectIdUnidad);
        utils\IConnection::execSql("CALL consumePuntos('Consume', '" . $IdU["codigo"] . "', $Ticket);");
    ...
    function ingresaBonificacionOld($CntPuntos, $IdUnidad, $Ticket) {
