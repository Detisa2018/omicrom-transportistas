...
class MensajesDAO implements FunctionsDAO {
    ...
    public function cuestionarioOmicrom() {
        $Select = "SELECT id,id_orden FROM ordenes_atendidas WHERE id_user_respuesta = 0 ORDER BY id ASC LIMIT 1";
        if (($query = $this->conn->query($Select)) && ($rs = $query->fetch_assoc())) {
            $res[1] = $rs["id"];
            $res[2] = $rs["id_orden"];
            return $res;
        }
