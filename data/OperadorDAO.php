...
class OperadorDAO implements FunctionsDAO {
    ...
    public function create($objectVO = OperadorVO) {
        ...
        if (($ps = $this->conn->prepare($sql))) {
            $ps->bind_param("sssss",
                    ...
                    $objectVO->getRegistro_fiscal(),
                    $objectVO->getRecidencia_fiscal()
            );
            if ($ps->execute()) {
                $id = $ps->insert_id;
                $ps->close();
    ...
    public function fillObject($rs) {
        ...
        if (is_array($rs)) {
            ...
            $objectVO->setRegistro_fiscal($rs["registro_fiscal"]);
            $objectVO->setRecidencia_fiscal($rs["recidencia_fiscal"]);
        }
        return $objectVO;
    }
