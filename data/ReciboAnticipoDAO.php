...
//receptor
    ...
    private function conceptos() {
        ...
        $Clave = "SELECT valor FROM variables_corporativo WHERE llave = 'ClaveAnticiposCreditos';";
        if (($qryClv = $this->mysqlConnection->query($Clave)) && ($vl = $qryClv->fetch_assoc())) {
            $concepto->setClaveProdServ($vl["valor"]);
        }
