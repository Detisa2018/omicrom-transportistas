    ...
    private function doctosRelacionados() {
        ...
        $sql = "
            SELECT 
                ...
                fc.total-IFNULL( IF(pagose.id <> " . $this->folio . ",  cxc.importe,0), 0 )-IFNULL( nc.importe, 0 ) ImpSaldoAnt, 
                fc.total-IFNULL( IF(pagose.id <> " . $this->folio . ",  cxc.importe,0), 0 )-IFNULL( nc.importe, 0 ) - pagose.importe ImpSaldoInsoluto, 
                        ...
                        if ($queryDes = $this->mysqlConnection->query($sqlDesgloseIeps)) {
                            ...
                            if (!($rs1["descloseIEPS"] === "N")) {
                                if ($rss['tax_ieps'] > 0) {
                                    ...
                                    $doctoRelacionadoTraDRieps->setImporteDR(number_format($doctoRelacionadoTraDRieps->getBaseDR() * $doctoRelacionadoTraDRieps->getTasaOCuotaDR(), 2, '.', ''));
...
//                                    $doctoRelacionadoTrasDR->setTrasladoDR($TraDrieps);
                ...
                if ($TaxIva > 0) {
                    ...
                    $TraDRiva = array();
                    $ImpPorcentaje = $BaseDR * 0.16;
                    $LimitSup = $ImpPorcentaje + 0.2;
                    $LimitInf = $ImpPorcentaje - 0.2;
                    $TaxIvaG = $TaxIva * $porcentajeT;
                    $PorcenteIva = ($LimitSup > $TaxIvaG && $LimitInf < $TaxIvaG) ? 0.160000 : 0.080000;
                    $doctoRelacionadoTraDRiva->setBaseDR($BaseDR);
                    $doctoRelacionadoTraDRiva->setImpuestoDR('002');
                    $doctoRelacionadoTraDRiva->setTasaOCuotaDR(number_format($PorcenteIva, 6));
                    $doctoRelacionadoTraDRiva->setTipoFactorDR('Tasa');
                    $doctoRelacionadoTraDRiva->setImporteDR(number_format($TaxIvaG, 2, '.', ''));
        ...
        $sqlIeps = "
            ...
            FROM (
                SELECT 
                    ...
                    ROUND( ( fcd.importe/fcd.preciob ), 4) cantidad,
                    fcd.preciob,
                    round(pagose.importe/fc.total ,4) porcentaje
                FROM fcd INNER JOIN fc ON fcd.id = fc.id
...
// Registrar Banco Emisor
            ...
            $totales->setTotalTrasladosBaseIVA8(number_format($this->getTotalTrasladosBaseIVA16(), 2, '.', ''));
            $totales->setTotalTrasladosImpuestoIVA8(number_format($this->getTotalTrasladosImpuestosIVA16(), 2, '.', ''));
            if ($this->getTotalRetencionesISR() > 0) {
