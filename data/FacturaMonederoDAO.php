...
//. "ROUND( ( fcd.subtotal + diferencia + ROUND( IF( cli.desgloseIEPS = 'S', 0.0000, fcd.tax_ieps ), 4 ) ) / ( fcd.preciou + ROUND( IF( cli.desgloseIEPS = 'S', 0.0000, fcd.factorieps ), 4 ) ), 4 ) Cantidad, "
        ...
        if (($query = $this->mysqlConnection->query($sql))) {
            ...
            while (($rs = $query->fetch_assoc())) {
                ...
                $EstacionNo = "SELECT numestacion FROM cia;";
                $RsNo = \com\softcoatl\utils\IConnection::execSql($EstacionNo);
                $DataFaltante = "SELECT pagos.uuid, pagos.serie, cli.observaciones, cre.permiso permisocre 
                FROM fc LEFT JOIN pagos ON pagos.id=fc.relacioncfdi 
                LEFT JOIN cli ON cli.id=pagos.cliente LEFT JOIN permisos_cre cre ON true 
                WHERE fc.id = " . $this->folio . " AND cre.catalogo='VARIABLES_EMPRESA' AND cre.llave = 'PERMISO_CRE';";
                $FtRs = \com\softcoatl\utils\IConnection::execSql($DataFaltante);
                $TextoMonederoDiarioOfi = $FtRs["permisocre"] . "  " . $FtRs["uuid"] . " " . $FtRs["observaciones"];
                $concepto->setDescripcion($TextoMonederoDiarioOfi);
