    ...
    <body class="A4-Ticket">
        ...
        <div id="MiTicket" class="sheet padding-05mm hideAll">
            ...
            <div align="center" class="text" style="align-items: flex-start">
                ...
                if ($Vt["fae"] === "Si" && $St) {
                    try {
                        ...
                        if ($VC4["valor"] === "com.detisa.omicrom.utils.qr.G500QRGenerator") {
                            $SqlB = "SELECT   to_base64( CONCAT( cu.permiso, '|', ( UNIX_TIMESTAMP( rm.fin_venta ) * 1000 ), '|' , rm.id, '|', qr.qrc, '|', 
                                        CAST( rm.pesos AS DECIMAL ( 15, 2 ) ), '|',
                                        CASE WHEN com.cve_producto_sat = 'PR07' AND com.cve_sub_producto_sat = 'SP16' THEN 'G Super' 
                                        WHEN com.cve_producto_sat = 'PR07' AND com.cve_sub_producto_sat = 'SP17' THEN 'G Premium' 
                                        WHEN com.cve_producto_sat = 'PR03' AND com.cve_sub_producto_sat = 'SP18' THEN 'G Diesel'  ELSE '' END, '|', 
                                        CAST( rm.volumen AS DECIMAL( 15, 3 ) ) ) ) QR 
                                        FROM rm LEFT JOIN com ON rm.producto = com.clavei JOIN v_rm_qrfae qr ON qr.id = rm.id 
                ...
                $SqlPoints = "SELECT msgfae FROM omicrom.variables";
                $Mensaje = utils\IConnection::execSql($SqlPoints);
                ?>
                <table style="width: 90%;margin-top: 5px;" >
                    <tr><th colspan="2"><?= $Mensaje["msgfae"] ?></th></tr>
                </table>
            </div>
        </div>
