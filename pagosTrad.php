...
require "services/PagosTradService.php";
...
$paginador = new Paginador(5, "", "", "", "", "idfae", "idfae", "", "ASC", 0, "REGEX", "pagosTra.php");
...
//    echo print_r($session, true);
    ...
    $paginador = new Paginador($Id,
            "pagose.idnvo,pagose.total_nc,pagose.importe,descuento",
            ", (
                SELECT p.id,p.idnvo,p.factura,p.fecha,p.total,p.importe,p.abonos,IFNULL(nc.total_nc,0) total_nc,p.folio,descuento
                FROM( 
                    SELECT pagose.id, pagose.idnvo, pagose.factura, fc.folio, fc.fecha, fc.total, pagose.importe, sub.abonos,fc.descuento
                    FROM 
                    pagose,fc,(
...
<html lang="es" xml:lang="es">
    <head>
        ...
        <title><?= $Gcia ?></title>
        <?= $lBd ? "<meta http-equiv=\"refresh\" content=\"2;url=pagosTrad.php?op=Genera\" />" : "" ?>
        <?= $lBd_2 ? "<meta http-equiv=\"refresh\" content=\"2;url=pagosTrad.php?op=generaReciboAnticipo\" />" : "" ?>
        <?= $lBd_4 ? "<meta http-equiv=\"refresh\" content=\"2;url=pagosTrad.php?op=generaNotaCredito&UsoCfdi=" . $request->getAttribute("UsoCfdi") . "\" />" : "" ?>
        <?= $lBd_5 ? "<meta http-equiv=\"refresh\" content=\"2;url=pagosTrad.php?op=generaComplementoCli&total=$importe\" />" : "" ?>
    ...
    <body>
        ...
        <?php if ($clienteVO->getTipodepago() === TiposCliente::PREPAGO && utils\HTTPUtils::getSessionValue("ComplementoPago") == 0) { ?>
            <form name="form1" method="get" action="pagosTrad.php">
                                ...
                                <tbody>
                                    <tr style="background-color: #E1E1E1; text-align: center;" class="texto_tablas">
                                        ...
                                        <?php if ($pagoVO->getStatus_pago() == StatusPagoPrepago::CON_ANTICIPO) { ?>
                                            ...
                                            $cSqlFC = "SELECT uuid,(total - IFNULL(descuento,0) ) total  FROM fc WHERE relacioncfdi = " . $pagoVO->getId() . " AND tdoctorelacionado = 'ANT' AND uuid <> '-----' AND status = " . StatusFactura::CERRADO;
                                            ...
                                            <?php if ($pagoVO->getStatus_pago() == StatusPagoPrepago::CON_FACTURA_CONSUMOS) { ?>
                                                ...
                                                <td style="background-color: #acecaa" class="ProcesoDif">
                                                    ...
                                                    if ($pagoVO->getImporte() > $FC["total"]) {
                                                        ...
                                                        $BuscaPago = "SELECT id FROM pagos where cliente = " . $pagoVO->getCliente() . " AND concepto = 'PREPAGO  SALDO A FAVOR' AND importe = $Dif AND aplicado = $Dif LIMIT 1;";
                                                        $Bp = utils\IConnection::execSql($BuscaPago);
                                                        if ($Bp["id"] > 0) {
                                                            echo "Saldo enviado al pago no." . $Bp["id"];
                                                        } else {
                                                            ?>
                                                            <a href="pagosTrad.php?op=AddDif&IdPago=<?= $pagoVO->getId() ?>&Dif=<?= $Dif ?>">Crear nuevo pago por la diferencia de $<?= number_format($Dif, 2) ?></a>
                                                            <?php
                                                        }
                                                    } else if ($pagoVO->getSaldoFavor() > 1) {
                                                        ?>
                                                        <strong class="Acceso">
                                                            <a href="pagosTrad.php?cVarVal=<?= $pagoVO->getSaldoFavor() ?>">Relación: <?= $pagoVO->getSaldoFavor() ?></a> 
                                                        </strong>
                                                        <?php
                                                    }
                                                    ?>
                                        ...
                                        if ($op === "generaComplemento") {
                                            ...
                                            $selectFcRelacionado = "SELECT total - descuento total FROM fc WHERE relacioncfdi = " . $pagoVO->getId() . " AND tdoctorelacionado = 'ANT' AND uuid <> '-----' AND  status = " . StatusFactura::CERRADO;
                                            ...
                                            if ($pagoVO->getImporte() < $fcResult['total'] && $pagoVO->getRelacion() == 0 && $pagoVO->getStatus_pago() == 5 && $total >= 2) {
                                                ?>
                        ...
                        <td>
                            <div  style="text-align: center;width: 100%">
                                <table style="margin-left: 1%;width: 98%;"  class="paginador" summary="Facturas relacionadas">
                                    ...
                                    <?php
                                    $BuscaExistencia = "SELECT id_fc,rc.uuid,rc.importe,folio,fc.serie,rc.uuid_relacionado,rc.id FROM relacion_cfdi rc LEFT JOIN fc ON fc.id=rc.id_fc WHERE  uuid_relacionado = '" . $pagoVO->getUuid() . "'";
                                    ...
                                    foreach ($rsB as $b) {
                                        ...
                                        $SqlFc = "SELECT uuid,status FROM  fc WHERE id=" . $b["id_fc"];
                                        $FcSql = utils\IConnection::execSql($SqlFc);
                                        if ($FcSql["uuid"] === "-----" && $FcSql["status"] == 3) {
                                            $uuidAnt = $b["uuid"];
                                            $uuidMod = explode('-', $uuidAnt);
                                            $uuidMod = $uuidMod[0] . "-" . $uuidMod[1] . "-" . $uuidMod[2] . "-XXLX-" . $uuidMod[4];
                                            $uuidAntP = $b["uuid_relacionado"];
                                            $uuidModP = explode('-', $uuidAntP);
                                            $uuidModP = $uuidModP[0] . "-" . $uuidModP[1] . "-" . $uuidModP[2] . "-XXLX-" . $uuidModP[4];
                                            $Update = "UPDATE relacion_cfdi SET uuid = '$uuidMod', uuid_relacionado = '$uuidModP' WHERE id = " . $b["id"];
                                            utils\IConnection::execSql($Update);
                                            ?>
                                            <input type="hidden" name="Return" id="Return" value="recargar">
                                            <?php
                                        }
                                        ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php
                                                if ($FcSql["uuid"] !== "-----") {
                                                    if ($NcSql["uuid"] !== '-----' && $NcSql["uuid"] !== null && $FcSql["uuid"] !== "-----") {
                                                        ?>
            ...
            if ($pagoVO->getStatus() === StatusPago::ABIERTO) {
                ...
                <table style="width: 100%; border: 2px solid; padding: 2px;" aria-hidden="true">
                    ...
                    <tr style="background-color: #E1E1E1; text-align: center;">
                        ...
                        <td>
                            <a href="pagosTrad.php?op=CerrarGenerar" title="Con esto timbras el recibo y das por cerrado el pago"  class="seleccionar">
                    ...
                    <tbody>
                        ...
                        while ($paginador->next()) :
                            ...
                            <tr>
                                ...
                                <td><?= $row["fecha"] ?></td>
                                <td style="text-align: right;"><?= number_format($row["total"] - $row["descuento"], 2) ?></td>
                                ...
                                <td style="text-align: right;"><?= number_format($row["total"] - $row["descuento"] - ($row["abonos"] + $row["total_nc"]), 2) ?></td>
                                <td style="text-align: center;">
                                    <?php if ($pagoVO->getStatus() === StatusPago::ABIERTO) { ?>
        ...
        if ($pagoVO->getStatus() === StatusPago::ABIERTO && $pagoVO->getAplicado() > 0) {
            $returnLink = urlencode("pagosTrad.php");
            $backLink = urlencode("pagosTrad.php?criteria=ini");
            if ($clienteVO->getTipodepago() === TiposCliente::CREDITO || (utils\HTTPUtils::getSessionValue("ComplementoPago") == 1 && $clienteVO->getTipodepago() !== TiposCliente::TARJETA && $clienteVO->getTipodepago() !== TiposCliente::MONEDERO)) {
                ...
                $(".GeneraNcIndp").click(function () {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            url = "pagosTrad.php?op=TimbraNcFc&TotalFc=" + this.dataset.totalfc + "&IdFc=" + this.dataset.idfc + "&FechaEmision=" + $("#swal-input4").val();
                            console.log(url);
            ...
            function getResultado(val_Json) {
                ...
                if (val_Json.Sucess) {
                    if (val_Json.IdOrigen === 0) {
                        /*Anticipos*/
                        url = "pagosTrad.php?op=generaAnticipo&busca=<?= $pagoVO->getId() ?>&FechaAnticipo=" + document.getElementById('swal-input3').value;
                    } else {
                        if (document.getElementById('swal-input1').value === "G02") {
                            addUrl = "&UsoCfdi=Si";
                        }
                        /*Notas de Credito*/
                        url = "pagosTrad.php?op=generaNota&busca=<?= $pagoVO->getId() ?>&FechaAnticipo=" + document.getElementById('swal-input2').value + addUrl;
                    }
                    document.location.href = url;
                }
            }
