...
require_once 'services/PagosServiceTra.php';
...
//                    $(".MuestraActualizar").hide();
    ...
    <body>
        ...
        <table style="width: 100%;" class="texto_tablas" aria-hidden="true">
            <tr>
                <td style="text-align: center;" class="nombre_cliente">
                    <a href="pagosTra.php"><div class="RegresarCss " alt="Flecha regresar" style="">Regresar</div></a>
                </td>
                    ...
                    <form name = "form1" id = "form1" method = "post" action = "">
                        ...
                        <table style = "width: 95%; border: 0px; padding: 0px;" class = "texto_tablas" aria-hidden = "true">
                            ...
                            if (is_numeric($busca)) {
                                ...
                                if ($Tc["tipodepago"] === "Monedero") {
                                    $SqlFc = "SELECT total FROM fc WHERE relacioncfdi = " . $pagoVO->getId()
                                            . " AND cliente =  " . $pagoVO->getCliente() . " ORDER BY id DESC;";
                                    $FcRelacion = utils\IConnection::execSql($SqlFc);
                                    $Psss = number_format($FcRelacion["total"], 1) == number_format($pagoVO->getImporte(), 1) ? false : true;
                                } else {
                                    $Psss = true;
                                }
                                //if ((($pagoVO->getUuid() === "-----" && $pagoVO->getStatusCFDI() == StatusPagoCFDI::ABIERTO) || $pagoVO->getStatus() === StatusPago::ABIERTO ) || ($pagoVO->getStatus() === StatusPago::CERRADO && $Tc["tipodepago"] === "Tarjeta") || $Psss) {
                                $SqlPagoInCxc = "SELECT id FROM omicrom.cxc where cliente = " . $pagoVO->getCliente() . " AND tm='H' AND referencia = " . $pagoVO->getId() . ";";
                                $rsCxc = utils\IConnection::execSql($SqlPagoInCxc);
                                if (!($rsCxc["id"] > 0)) {
                                ...
                                <tr>
                                    <td colspan="2">
                                        <div id="Add_New_Importes"></div>
                                    </td>
                                </tr>
                                <tr style="height: 50px;">
                                    <td style="text-align: right;"><input type='submit' name='Boton' value='Agregar' class='nombre_cliente'></td>
                                    <td  style="text-align: center;">
                                        <?php if ($clienteVO->getTipodepago() === "Tarjeta") { ?>
                                            <input type='button' name='MultiPagos' id="MultiPagos" value='Multi-Pagos'>
                                        <?php } ?>
                                    </td>
                                </tr>
                                <?php
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                $("#MultiPagos").click(function () {
                    Swal.fire({
                        icon: "question",
                        iconColor: "#5499C7",
                        title: "Cantidad de registros de pagos que se desean registrar",
                        html: `
                    <select id="cntPagos" name='cntPagos' class="swal2-select">
                        <option value="2">2 Registro</option>
                        <option value="3">3 Registro</option>
                        <option value="4">4 Registro</option>
                        <option value="5">5 Registro</option>
                        <option value="6">6 Registro</option>
                        <option value="7">7 Registro</option>
                        <option value="8">8 Registro</option>
                    </select>
                `
                    }).then((result) => {
                        if (result.isConfirmed) {
                            var e;
                            var htmlAdd = "<table style='width:100%;' class='texto_tablas'>";
                            for (e = 2; e <= $("#cntPagos").val(); e++) {
                                $("#Add_New_Importes").html("Cnt " + e);
                                htmlAdd += "<tr class='texto_tablas'><td style='background-color: #e1e1e1; text-align: right;width: 25%;' class='nombre_cliente'><strong>No. " + e + " Importe</strong></td>\n\
                                                <td><input type='text' name='ImporteNo" + e + "' id='ImporteNo" + e + "' style='width:95px;margin-left: 5px;'></td></tr>";
                                htmlAdd += "<tr class='texto_tablas'><td style='background-color: #e1e1e1; text-align: right;width: 25%;' class='nombre_cliente'><strong>No. " + e + " Concepto</strong></td>\n\
                                                <td><input type='text' name='ConceptoNo" + e + "' id='ConceptoNo" + e + "' style='width:300px;margin-left: 5px;'></td></tr>";
                            }
                            htmlAdd += "</table>";
                            htmlAdd += "<input type='hidden' name='CantidadPagos'  id='CantidadPagos' value='" + $("#cntPagos").val() + "'>";
                        }
                        $("#Add_New_Importes").html(htmlAdd);
                    });
                });
                var MontoNeto = "";
                var i = 0;
