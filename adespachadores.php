...
$paginador = new Paginador($Id,
        "man.id,MAX(man.inventario) inventario , man.lado,man.isla_pos,GROUP_CONCAT(DISTINCT man.posicion ORDER BY man.posicion ASC) posicion , 
    ...
    <body>
        ...
        <div id="TablaDatos">
            <table class="paginador" aria-hidden="true">
                <?php
                echo $paginador->headers(array("Isla", "Posición", "Despachador asignado", "Cambiar despachador a", "Revertir despachador", "Despachador Sig.", "Reasignar Desp. Sig.", "Inv", "Posicion(es)"), array());
                while ($paginador->next()) {
                    $row = $paginador->getDataRow();

                    $VenPos = "Despachador" . $row["isla_pos"];
                    $VenSig = "DespachadorSig" . $row["isla_pos"];
                    $SqlDd = "SELECT GROUP_CONCAT(DISTINCT man.posicion ORDER BY man.posicion ASC) posiciones  FROM omicrom.man 
                                    WHERE activo = 'Si' AND inventario='Si' AND isla_pos = " . $row["isla_pos"] . " GROUP BY isla_pos";
                    $Posiciones_Cn_Inv = utils\IConnection::execSql($SqlDd);
                    ?>
                    <tr class="cks">
                        ...
                        <td><?= $Posiciones_Cn_Inv["posiciones"] ?></td>
                    </tr>
                    <?php
                }
                ?>
