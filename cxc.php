...
if ($request->getAttribute("criteria") === "ini") {
    $SqlVc = "SELECT valor FROM variables_corporativo WHERE llave = 'IniciaCxcComun'";
    $Vc = utils\IConnection::execSql($SqlVc);
    if (isset($Vc["valor"])) {
        $cRef = $cFec = $cFac = "";
        if ($Vc["valor"] === "referencia") {
            $cRef = "checked";
        } elseif ($Vc["valor"] === "fecha") {
            $cFec = "checked";
        } elseif ($Vc["valor"] === "factura") {
            $cFac = "checked";
        }elseif ($Vc["valor"] === "corte") {
            $cCort = "checked";
        }
    }
}
            ...
            <table class="cxcOpciones" aria-hidden="true">
                ...
                <tr class="texto_tablas">
                    ...
                    <td align="center">
                        ...
                        &nbsp; <input type="radio" class="botonAnimatedMin" name="orden" value="corte" <?= $cCort ?> onChange=submit(); > Corte
                    </td>
                </tr>
            </table>
