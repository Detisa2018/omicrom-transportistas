    ...
    <head>
        ...
        <script>
            $(document).ready(function () {
                ...
                $("#Consig").val("<?= $Consig ?>");
                if ("<?= $sanitize->sanitizeString("Concentrado") ?>" === "on") {
                    $(".InformaciónDetallada").hide();
                    $("#Concentrado").prop("checked", true);
                }
            });
        </script>
    </head>
    ...
    <body>
        <div id='container'>
            ...
            <div id="Reportes" style="min-height: 200px;">
                <?php if (empty($Corte)) { ?>
                    <table aria-hidden="true" class="InformaciónDetallada">
                        <thead>
                            <tr>
                                <td>Fecha</td>
                                <td>Producto</td>
                        ...
                        <tbody>
                            ...
                            foreach ($registros as $rg) {
                                $ProductoLitros[$rg["descripcion"]] += $rg["volumen"];
                                $ProductoVentas[$rg["descripcion"]] += $rg["ventas"];
                                $ProductoImporte[$rg["descripcion"]] += $rg["importe"];
                                $ProductoIeps[$rg["descripcion"]] += $rg["iepsCuota"];
                                $ProductoIva[$rg["descripcion"]] += $rg["iva"];
                                $ProductoDescuento[$rg["descripcion"]] += $rg["descuento"];
                                $ProductoTotal[$rg["descripcion"]] += $rg["total"] - $rg["descuento"];
                                if ($rg["total"] > 0) {
                    ...
                    <table>
                        <thead>
                            <tr>
                                <td colspan="100%">Concentrado por producto</td>
                            </tr>
                            <tr>
                                <td>Producto</td>
                                <td>Ventas</td>
                        ...
                        <tbody>
                            <?php
                            $TtVolumen = $TtImporte = $TtIEPS = $TtIva = $TtDesc = $TtTotal = 0;
                            foreach ($ProductoLitros as $kpl => $pl) {
                                ?>
                                <tr>
                                    <td><?= $kpl ?></td>
                                    <td style="text-align: right;"><?= $ProductoVentas[$kpl] ?></td>
                                    <td style="text-align: right;"><?= number_format($pl, 2) ?></td>
                                    <td style="text-align: right;">$<?= number_format($ProductoImporte[$kpl], 2) ?></td>
                                    <td style="text-align: right;">$<?= number_format($ProductoIeps[$kpl], 2) ?></td>
                                    <td style="text-align: right;">$<?= number_format($ProductoIva[$kpl], 2) ?></td>
                                    <td style="text-align: right;">$<?= number_format($ProductoDescuento[$kpl], 2) ?></td>
                                    <td style="text-align: right;">$<?= number_format($ProductoTotal[$kpl], 2) ?></td>
                                </tr>
                                <?php
                                $TtVolumen += $pl;
                                $TtImporte += $ProductoImporte[$kpl];
                                $TtIEPS += $ProductoIeps[$kpl];
                                $TtIva += $ProductoIva[$kpl];
                                $TtDesc += $ProductoDescuento[$kpl];
                                $TtTotal += $ProductoTotal[$kpl];
                                $TtVentas += $ProductoVentas[$kpl];
                            }
                            ?>
                        </tbody>
                        <tfoot>
                            <tr>
                                <td></td>
                                <td><?= number_format($TtVentas, 0) ?></td>
                                <td><?= number_format($TtVolumen, 2) ?></td>
                                <td>$<?= number_format($TtImporte, 2) ?></td>
                                <td>$<?= number_format($TtIEPS, 2) ?></td>
                                <td>$<?= number_format($TtIva, 2) ?></td>
                                <td>$<?= number_format($TtDesc, 2) ?></td>
                            ...
                            <td>
                                <table style="width: 100%" aria-hidden="true">
                                    <tr>
                                        ...
                                        <td>Concentrado : <input type="radio" name="Concentrado" id="Concentrado" class="botonAnimatedMin"></td>
                                    <tr>
