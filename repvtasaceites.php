...
$IvaSql = "SELECT iva FROM cia;";
$Iva = utils\IConnection::execSql($IvaSql);
                ...
                <div id="Reportes">
                    ...
                    if ($Detallado === "No") {
                        ...
                        <table id="RpAceites2" aria-hidden="true" class="display" style="width: 100%;">
                            <thead>
                                <tr>
                                    <td colspan="11" style="text-align: center;font-weight: bold;font-size: 20px;">Venta por producto</td>
                                </tr>
                                <tr style="font-weight: bold;">
                                    <td>Clave</td>
                                    <td>Descripcion</td>
                            ...
                            <tbody>
                                <?php
                                $nCnt = $nImp = $nCos = $CostoConIVA = 0;
                                foreach ($registrosP as $rg) {
                                    ?>
                                    <tr>
                                        <td><?= $rg["clave_producto"] ?></td>
                                        <td><?= $rg["descripcion"] ?></td>
                                        <td style="text-align: right;"><?= number_format($rg["cantidad"], 0) ?></td>
                                        <td style="text-align: right;"><?= number_format($rg["costo"] / $rg["cantidad"], 2) ?></td>
                                        <td style="text-align: right;"><?= number_format(($rg["costo"] * (1 + ($Iva["iva"] / 100))) / $rg["cantidad"], 2) ?></td>
                                        <td style="text-align: right;"><?= number_format(($rg["costo"] * (1 + ($Iva["iva"] / 100))), 2) ?></td>
                                        <td style="text-align: right;"><?= number_format($rg["unitario"], 2) ?></td>
                                        <td style="text-align: right;"><?= number_format($rg["importe"] / 1.16, 2) ?></td>
                                        <td style="text-align: right;"><?= number_format(($rg["importe"] / 1.16) * 0.16, 2) ?></td>
                                        <td style="text-align: right;"><?= number_format($rg["importe"], 2) ?></td>
                                        <td style="text-align: right;"><?= number_format($rg["importe"] - ($rg["costo"] * (1 + ($Iva["iva"] / 100))), 2) ?></td>
                                    </tr>
                                    <?php
                                    $CostoConIVA += ($rg["costo"] * (1 + ($Iva["iva"] / 100))) / $rg["cantidad"];
                                    $nCnt += $rg["cantidad"];
                                    $nUn += $rg["unitario"];
                                    $nCosNto += $rg["costo"] / 1.16;
                                    $nCosIVA += ($rg["costo"] / 1.16) * 0.16;
                                    $nCos += ($rg["costo"] * (1 + ($Iva["iva"] / 100)));
                                    $nImpNto += $rg["importe"] / 1.16;
                                    $nImpIva += ($rg["importe"] / 1.16) * 0.16;
                                    $nImp += $rg["importe"];
                                    $nDif += $rg["importe"] - ($rg["costo"] * (1 + ($Iva["iva"] / 100)));
                                }
                                ?>
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td></td>
                                    <td></td>
                                    <td style="text-align: right;"><?= number_format($nCnt, 0) ?></td>
                                    <td style="text-align: right;"></td>
                                    <td style="text-align: right;"><?= number_format($CostoConIVA, 2) ?></td>
                                    <td style="text-align: right;"><?= number_format($nCos, 2) ?></td>
                                    <td style="text-align: right;"></td>
                                    <td style="text-align: right;"><?= number_format($nImpNto, 2) ?></td>
                                    <td style="text-align: right;"><?= number_format($nImpIva, 2) ?></td>
                                    <td style="text-align: right;"><?= number_format($nImp, 2) ?></td>
                                    <td style="text-align: right;"><?= number_format($nDif, 2) ?></td>
                                </tr>
                            </tfoot>
                        </table>
