...
$Cuestionario = $mensajeDAO->cuestionarioOmicrom();
...
if ($Cuestionario[2] > 0) {
    ?>
                var TextHtml = "<table style='width:100%;'>"
                        + "<tr><td>Malo:</td><td><input type='checkbox' name='Malo' id='Malo' class='botonAnimated'></td>"
                        + "<td>Bueno:</td><td><input type='checkbox' name='Bueno' id='Bueno' class='botonAnimated'></td>"
                        + "<td>Excelente:</td><td><input type='checkbox' name='Excelente' id='Excelente' class='botonAnimated'></td></tr>"
                        + "<tr><td colspan='6'>Observaciones: <textarea name='Observaciones' id='Observaciones' style='heigth:150px;width:250px;'></textarea></td></tr>"
                        + "</table>";
                Swal.fire({
                    title: "¡Ayúdanos a ofrecerte un mejor servicio! Orden no.<?= $Cuestionario[2] ?> atendida",
                    showConfirmButton: true,
                    showCancelButton: true,
                    confirmButtonText: "Calificar",
                    html: TextHtml
                }).then((result) => {
                    var Malo = $('#Malo').prop('checked');
                    var Bueno = $("#Bueno").prop('checked');
                    var Excelente = $("#Excelente").prop('checked');
                    console.log(Malo + " " + Bueno + " " + Excelente);
                    if (result.isConfirmed) {
                        if (!((Malo && Bueno) || (Malo && Excelente) || (Bueno && Malo) || (Bueno && Excelente) || (Excelente && Malo) || (Excelente && Bueno))) {
                            var res;
                            if (Malo) {
                                res = 1;
                            } else if (Bueno) {
                            ...
                            jQuery.ajax({
                                ...
                                data: {
                                    "Op": "ResultadoReporte",
                                    "id_Orden": <?= $Cuestionario[1] ?>,
                                    "Calificacion": res,
                                    "Observacion": $("#Observaciones").val(),
                                    "Usr": "<?= $usuarioSesion->getId() ?>"
                                },
                            ...
                            Swal.fire({
                                title: "Favor de solo leccionar solo una opcion",
                                showConfirmButton: true,
                                showCancelButtoun: false,
