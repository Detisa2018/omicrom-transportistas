                            ...
                            <tbody>
                                ...
                                foreach ($registros as $rg) {
                                    ...
                                    if (!empty($clave) && $clave !== $rg["clavei"]) {
                                        if ($Informacion == TipoInformacion::OMICROM || $Informacion == TipoInformacion::COMPARATIVO) {
                                            ...
                                            <tr class="subtotal">
                                                ...
                                                <?php
                                                if ($incluir) {
                                                    echo $busca == 1 ? '<td class="SelectCompensado">' . number_format($VtaExtra / $nFac, 3) . '</td>' : ' <td class="SelectCompensado">' . number_format($VtaExtra / $nFac, 3) . '</td>';
                                                }
                                                ?>
                                                ...
                                                <td class="SelectCompensado"><?= number_format($InvFinalNC / $nFac, 3) ?></td>
                                                <td class="SelectCompensado"><?= number_format($DiferenciaCompensado, 3) ?></td>
                                                <td class="SelectCompensado"><?= number_format(($DiferenciaCompensado * 100) / ($InvFinalNC / $nFac), 2) ?></td>
                                            </tr>
                                            <?php
                                            $PorcentajeAcumuladoComp = $VolumenDevuelto = $InvFinalNC = $PorcentajeAcumulado = $Ccomp = $InvTeorico = $Ventas = $DiferenciaCompensado = $Venta_consignacion = $DiferenciaBruto = $Cargas = $VentaCompensadaConsig = $VentaCompensada = $VtaExtra = $InvFinal = $VentasCV = $CargasCV = $Bruto = $Diferencia = $Jarreos = $InicialCompensado = $InvFinalComp = 0;
                                        } else {
                                            ?>
                                            <tr class="subtotal">
                                                <td colspan="2">Resumen</td>
                                    ...
                                    if ($Informacion == TipoInformacion::OMICROM || $Informacion == TipoInformacion::COMPARATIVO) {
                                        ...
                                        $InvTeorico = $busca === "1" ? $rg["inventario_teorico"] + ($incluir ? $Rmd["cantidad"] : 0) : $rg["inventario_teorico_documentado"] + ($incluir ? $Rmd["cantidad"] : 0);
