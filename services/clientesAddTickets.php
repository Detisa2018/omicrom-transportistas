    ...
    try {
        if ($request->hasAttribute("Boton")) {
            if ($request->getAttribute("Boton") === utils\Messages::OP_ADD) {
                ...
                if (!in_array($request->getAttribute("idTicket"), $_SESSION["Ventas"])) {
                ...
                foreach ($_SESSION["Ventas"] as $vs => $val) {
                    ...
                    $InsertIntoCxc = "INSERT INTO cxc (cliente,placas,referencia,fecha,hora,tm,concepto,cantidad,importe,recibo,corte,producto,rubro,factura) "
                            . "SELECT $busca,'" . $Placas . "',id,'" . date("Y-m-d") . "','" . date("H:i:s") . "','C','Venta a cliente desde proceso addTickets',(importe/precio) volumen,importe,0,corte,producto,"
