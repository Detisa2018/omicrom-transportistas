...
if ($request->hasAttribute("Boton") && $request->getAttribute("Boton") !== utils\Messages::OP_NO_OPERATION_VALID) {
    ...
    try {
        ...
        if ($request->getAttribute("Boton") === utils\Messages::OP_ADD) {
            ...
            $usuarioVO = $usuarios->getUser($usuarioVO->getId());
            $StatusAnterior = $usuarioVO->getStatus();
            ...
            if ($CountBit["no"] == 0) {
                ...
                $Msj = "<br>El usuario ya tiene registros en bitacora de eventos y el nombre ni el usuario es posible modificar el nombre ni el usuario!";
            }
            $usuarioVO->setMail($sanitize->sanitizeEmail("Mail"));
            $usuarioVO->setRol($sanitize->sanitizeInt("Rol"));
            $usuarioVO->setStatus($sanitize->sanitizeString("Status"));
            ...
            if ($StatusAnterior !== $sanitize->sanitizeString("Status")) {
                $htmlAddG = $sanitize->sanitizeString("Status") === "inactive" ? "BAJA" : "ACTIVACION";
                BitacoraDAO::getInstance()->saveLog($usuarioSesion->getNombre(), "ADM", "$htmlAddG DE USUARIO : " . $usuarioVO->getNombre());
            }
...
if ($request->hasAttribute("op")) {
    ...
    try {
        ...
        } elseif ($request->getAttribute("op") === utils\Messages::DOWNLOAD) {
            if ($request->getAttribute("Accion") == 1) {
                $archivo = "/home/omicrom/xml/Carta_Responsiva.pdf";
            } else {
                $archivo = "/home/omicrom/xml/Formato_Solicitud_Accesos_Sistema_Omicrom.pdf";
            }

            $tamanoBytes = filesize($archivo);
            $tamanoKB = round($tamanoBytes / 1024, 2);
            echo "El archivo pesa: $tamanoKB KB";
