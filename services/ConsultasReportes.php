...
if (($result = $mysqli->query($selectTurnosActivos))) {
    while ($row = $result->fetch_array()) {
        $Turnos[$row['turno']] = $row['turno'];  // Uso de comillas alrededor de 'turno'
    }
}
...
error_log("parametros " . $Reporte . " " . $FechaI . " " . $FechaF);
...
elseif ($Reporte == 95) : /* Consulta reporte depositos bancarios */
    error_log("Entramos a la sección del reporte 95: depósitos bancarios con estos atributos: " . $FechaI . " " . $FechaF . " " . $Reporte);
        $cSql = "SELECT ct.fechaf fecha,ct.id corte,egr.clave,bancos.banco,bancos.cuenta, bancos.concepto cptcuenta,egr.concepto,egr.importe,ct.turno,egr.plomo, ct.statusctv FROM ct,egr LEFT JOIN bancos ON egr.clave = bancos.id WHERE ct.id = egr.corte AND DATE(ct.fecha) BETWEEN DATE('$FechaI') AND DATE('$FechaF') ORDER BY bancos.banco,ct.id";
...
elseif ($Reporte == 34) : /* Consulta para Pipas capturadas */
    ...
    if ($VcSc["valor"] == 1) {
        ...
        me.uuid,CONCAT(cli.id, '.- ' , cli.nombre, ' | ' , cli.observaciones , ' | ', cli.id_externo) cliente
        FROM com, prv, me 
        ...
        LEFT JOIN cxc ON cxc.referencia = cargas.id
...
elseif ($Reporte == 206) : /* Consulta reporte factura */
    $cSql = "SELECT rm.id,com.descripcion,rm.fin_venta,rm.posicion,ROUND((rm.importe/rm.precio),3) volumen ,ROUND(((rm.importe-(rm.ieps*rm.volumen))/(1.16)),2) importe,
                        ROUND(((rm.importe-(rm.ieps*rm.volumen))/(1.16))*(rm.iva),2) Iva,ROUND(rm.ieps*rm.volumen,2) ieps,
                        ROUND(rm.importe, 2) total
                                FROM rm inner join com on rm.producto = com.clavei
                                WHERE rm.fecha_venta like '$anio$mes%' 
                                AND rm.tipo_venta in ('D','N')
                                AND rm.importe > 0";
endif;