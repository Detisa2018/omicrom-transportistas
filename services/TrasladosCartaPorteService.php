...
try {
    error_log("MOSTRAMOS COMPROBANTE ===== " . print_r($request, true));
    if ($request->getAttribute("Comprobante") === "Nuevo") {
        ...
        $CpVO->setTabla("Traslados");
        error_log("CREAMOS CARTA PORTE");
        if ($id = $CpDAO->create($CpVO)) {
            error_log("SE CREO CARTA PORTE");
...
//            $RsVal = $SqlValida->fetch_array();
    ...
    } else if ($request->getAttribute("DireccionesOrigen") === utils\Messages::OP_ADD) {
        $InsertDireccion = "INSERT INTO carta_porte_destino (id_destino_fk,id_carta_porte_fk,fecha,distancia,tipo,origen) "
                . "VALUES ('" . $request->getAttribute("DireccionOrigen") . "','" . $CpVO->getId() . "','"
                . $request->getAttribute("HoraLlegadaOrigen") . "','','Origen','TRA')";
        if ($mysqli->query($InsertDireccion)) {
            $Msj = utils\Messages::RESPONSE_VALID_CREATE;
        }
    } else if ($request->getAttribute("DireccionesDestino") === utils\Messages::OP_ADD) {
        $InsertDireccion = "INSERT INTO carta_porte_destino (id_destino_fk,id_carta_porte_fk,fecha,distancia,tipo,origen) "
                . "VALUES ('" . $request->getAttribute("DireccionDestino") . "','" . $CpVO->getId() . "','"
                . $request->getAttribute("HoraLlegadaDestino") . "','" . $request->getAttribute("DistanciaDestino") . "','Destino','TRA')";
        if ($mysqli->query($InsertDireccion)) {
            $Msj = utils\Messages::RESPONSE_VALID_CREATE;
        }
        ...
        $TrasladosDAO = new TrasladosDAO();
        $TrasladosVO = new TrasladosVO();
        $TrasladosVO = $TrasladosDAO->retrieve($request->getAttribute("idTraslado"));
        $TrasladosVO->setFecha($request->getAttribute("Fecha"));
        $TrasladosDAO->update($TrasladosVO);
    } else if ($request->getAttribute("Boton") === "Agrega Producto") {
        error_log("CREAMOS TRASLADOS DETALLE ");
        $T_DetDAO = new TrasladosDetalleDAO();
        $T_DetVO = new TrasladosDetalleVO();
        $T_DetVO->setId($busca);
        $T_DetVO->setProducto($request->getAttribute("Producto"));
        $T_DetVO->setCantidad($request->getAttribute("Cantidad"));
        $T_DetVO->setPrecio(0);
        $T_DetVO->setPreciob(0);
        $T_DetVO->setIva(0);
        $T_DetVO->setIeps(0);
        $T_DetVO->setImporte(0);
        $T_DetDAO->create($T_DetVO);
    }
} catch (Exception $ex) {
