...
if ($request->hasAttribute("Boton") && $request->getAttribute("Boton") !== utils\Messages::OP_NO_OPERATION_VALID) {
    ...
    try {
        if ($request->getAttribute("Boton") === utils\Messages::OP_SAVE) {
            ...
            $idCliente = $partes[0];
            $BuscaCxc = "SELECT id FROM cxc WHERE referencia in (SELECT carga FROM me WHERE id = $busca)  AND cxc.tm='C';";
            $IdCxc = utils\IConnection::execSql($BuscaCxc);
            if ($IdCxc["id"] > 0) {
                $Cxc = "UPDATE cxc SET cliente = $idCliente WHERE referencia in (SELECT carga FROM me WHERE id = $busca)  AND concepto ='Compra de pipa, medio XML'  AND cxc.tm='C';";
                utils\IConnection::execSql($UpdateCxc);
            } else {
                $Cxc = "INSERT INTO cxc (cliente, placas,referencia,fecha,hora,tm,concepto,cantidad, importe,recibo,corte,producto,rubro,factura) "
                        . "SELECT $idCliente,'-', cargas.id,SUBSTRING_INDEX(cargas.fecha_inicio, ' ', 1),TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(cargas.fecha_inicio, ' ', 2), ' ', -1)),'C','Compra de pipa edicion',"
                        . "cargas.vol_final,me.importefac,0,0,(select clavei from com where com.descripcion = cargas.producto),'-----',0  FROM cargas LEFT JOIN me ON me.id=cargas.entrada "
                        . "WHERE cargas.id =  (SELECT carga FROM me WHERE id = $busca)";
                error_log($Cxc);
            }
            utils\IConnection::execSql($Cxc);
            $Return = "entradas.php?";
            $Msj = "Cliente reasignado";
        }
...
if ($request->hasAttribute("op")) {
    ...
    try {
        if ($request->getAttribute("op") == 2) {
            $ClienteDist = explode("|", $request->getAttribute("ClienteS"));
            utils\HTTPUtils::setCookieValue("Cliente", $ClienteDist[0]);
            $cargasVO = $cargasDAO->retrieve($carga);
