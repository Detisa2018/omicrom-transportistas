...
if ($request->hasAttribute("criteria")) {
    ...
    utils\HTTPUtils::setSessionValue("FechaHI", date("Y-m-d H:i:s"));
    utils\HTTPUtils::setSessionValue("FechaHF", date("Y-m-d H:i:s"));
...
if ($request->hasAttribute("FechaHI")) {
    utils\HTTPUtils::setSessionValue("FechaHI", $sanitize->sanitizeString("FechaHI"));
}
if ($request->hasAttribute("FechaHF")) {
    utils\HTTPUtils::setSessionValue("FechaHF", $sanitize->sanitizeString("FechaHF"));
}
...
$FechaHI = utils\HTTPUtils::getSessionValue("FechaHI");
$FechaHF = utils\HTTPUtils::getSessionValue("FechaHF");
...
$VarTipoLectura = "SELECT valor FROM variables_corporativo WHERE llave='IgualCorteDia'";
$RsVc = utils\IConnection::execSql($VarTipoLectura);
if ($RsVc["valor"] == 1) {
    $AddSql = "SUM(rm.importe / rm.precio) volumenp, 
                    SUM(rm.importe) pesosp,";
} else {
    $AddSql = "SUM(rm.volumenp) volumenp, 
...
if ($Desglose === "Cortes") {
    $selectRmDefault = "
                    ...
                    COUNT(rm.id) ventas,
                    AVG(rm.ieps) ieps,
                    SUM(rm.importe / rm.precio) volumen, 
                    SUM(rm.importe) pesos, 
...
$SelectByDiaPosicion = "SELECT sub.dispensario,sub.posicion,sub.producto,CONCAT('| ', sub.fecha, ' |') fecha, sub.ventas ventas, sub.pesos - IFNULL(subN.ProducidoP, 0) pesos, sub.volumen - IFNULL(subN.Producido, 0) volumen,
        subN.pesos pesosN, subN.volumen volumenN,sub.descuento,sub.descripcion
    FROM
        (SELECT rm.fecha,rm.corte,rm.ventas,rm.pesosp pesos,rm.volumenp volumen,rm.descuento,dispensario,posicion,producto,descripcion
        FROM
            (SELECT DATE(ct.fecha) fecha,rm.corte,rm.tipo_venta,rm.producto,com.descripcion,rm.precio precios,COUNT(rm.id) ventas,AVG(rm.ieps) ieps,
                SUM(rm.volumen) volumen,SUM(rm.pesos) pesos,COUNT(1) cantidadVenta,SUM(rm.volumenp) volumenp,SUM(rm.pesosp) pesosp,SUM(rm.descuento) descuento
                ,rm.dispensario,rm.posicion
        FROM com, rm, ct
        WHERE TRUE AND com.clavei = rm.producto AND com.activo = 'Si' AND rm.corte = ct.id AND rm.tipo_venta IN ('D') AND 
        DATE(ct.fecha) BETWEEN DATE('$FechaI') AND DATE('$FechaF')
        GROUP BY dispensario,posicion,producto) rm
        GROUP BY dispensario,posicion,producto) sub
            LEFT JOIN
        (SELECT rm.fecha,rm.corte,rm.ventas,rm.pesosp pesos,rm.volumenp volumen,rm.Producido,rm.ProducidoP,dispensario,posicion,producto,descripcion
        FROM
            (SELECT DATE(ct.fecha) fecha,rm.corte,rm.tipo_venta,rm.producto,com.descripcion,rm.precio precios,COUNT(rm.id) ventas,AVG(rm.ieps) ieps,SUM(rm.volumen) volumen,
                SUM(rm.pesos) pesos,COUNT(1) cantidadVenta,SUM(rm.volumen) volumenp,SUM(rm.pesosp) pesosp,sum(volumen) - sum(volumenp) Producido,
                SUM(rm.pesos - rm.pesosp) ProducidoP,SUM(rm.descuento) descuento,rm.dispensario, rm.posicion
        FROM com, rm, ct
        WHERE TRUE AND com.clavei = rm.producto AND com.activo = 'Si' AND rm.corte = ct.id AND rm.tipo_venta IN ('N') AND 
        DATE(ct.fecha) BETWEEN DATE('$FechaI') AND DATE('$FechaF')
        GROUP BY dispensario,posicion,producto) rm
        GROUP BY dispensario,posicion,producto) subN ON sub.fecha = subN.fecha
        GROUP BY sub.dispensario,sub.posicion,sub.producto,sub.fecha
    ";

$SelectByDiaPosicionT = "SELECT 
    sub.producto,
    sum(sub.ventas) ventas,
    sum(sub.pesos - IFNULL(subN.ProducidoP, 0)) pesos,
    sum(sub.volumen - IFNULL(subN.Producido, 0)) volumen,
    sum( subN.pesos )  pesosN,
    sum( subN.volumen ) volumenN,
    sum(sub.descuento) descuento,
    sub.descripcion
FROM
    (SELECT 
        rm.fecha,
    ...
    FROM
        com, rm, ct
    WHERE
        TRUE AND com.clavei = rm.producto
            AND com.activo = 'Si'
            AND rm.corte = ct.id
            AND rm.tipo_venta IN ('D')
            AND DATE(ct.fecha) BETWEEN DATE('$FechaI') AND DATE('$FechaF')
...
if ($Detallado === "Si") {
    if ($Turno === "No") {
        ...
        foreach ($Productos as $rg) {
            ...
            $ventasNP .= $ventasNP == "" ? "IFNULL($rg[clavei].cantidadVentaN$rg[id],0) cantidadVentaN$rg[id]" : ", IFNULL($rg[clavei].cantidadVentaN$rg[id],0) cantidadVentaN$rg[id]";
            ...
            if ($Desglose == "Cortes") {
                $Sql .= "
                        LEFT JOIN (
                           SELECT rm.corte, rm.fecha, rm.ventas ventas$rg[id], rm.producto,rm.ieps ieps$rg[id],
                           ((rm.pesosp  - (rm.ieps * (rm.volumenp  ) )) / 1.16) impSinIva$rg[id] , 
                            rm.pesosp  pesos$rg[id], rmN.Producido producido$rg[id],rmN.ProducidoP producidop$rg[id],
                            rm.volumenp  volumen$rg[id],
                            rm.cantidadVenta cantidadVenta$rg[id],
                            rmN.cantidadVenta cantidadVentaN$rg[id],
                            0 pesosN$rg[id],
                ...
                $Sql .= "
                        LEFT JOIN (
                            SELECT rm.corte, rm.fecha, rm.ventas ventas$rg[id], rm.producto,rm.ieps ieps$rg[id], ((rm.pesosp - (rm.ieps * (rm.volumenp))) / 1.16) impSinIva$rg[id] , 
                            rm.pesosp  pesos$rg[id], rmN.producido producido$rg[id],rmN.ProducidoP producidop$rg[id],
                            rm.cantidadVenta cantidadVenta$rg[id],
                            rmN.cantidadVenta cantidadVentaN$rg[id],
                            rm.volumenp  volumen$rg[id],
        ...
        if ($Desglose == "Cortes") {
            $Sql .= " 
            LEFT JOIN (
                SELECT vt.corte,DATE( ct.fecha ) fecha, SUM( vt.total  ) pesosA, SUM(vt.cantidad) cnt
            ...
            $Sql .= " 
            LEFT JOIN (
                SELECT DATE( vt.fecha ) fecha, SUM( vt.total  ) pesosA, SUM(vt.cantidad) cnt
        ...
        $selectByDia = "
            SELECT * FROM ( 
                SELECT  CONCAT('| ',ct.fecha,' |') fecha ,VT.cnt cntA, $ventasNP,$ventas,$pesosN,$volumenN, $pesos, $volumen,$ventasP, $ProducidoN,$ProducidopN,$ieps,$ImpSnIva,$DescuentoC,SUM(VT.pesosA) pesos_ace
                FROM $Sql
            ) sub 
            GROUP BY sub.fecha 
            ORDER BY sub.fecha";
    } else {
        $Igualcortedia = utils\IConnection::execSql("SELECT valor FROM variables_corporativo WHERE llave='IgualCorteDia");
        if ($Igualcortedia["valor"] == 1) {
            $HtmlAdd = " SUM(rm.importe/rm.precio) volumen, 
                    SUM(rm.importe) pesos, ";
        } else {
            $HtmlAdd = "SUM(rm.volumenp) volumen, 
                    SUM(rm.pesosp) pesos, ";
        }
        $selectByDia = "
            SELECT * FROM (
                SELECT sub.corte,  CONCAT('| ',sub.fecha,' |') fecha , sub.producto, sub.ventas, sub.pesos importe, sub.volumen volumen,VT.pesosA 
                FROM (
                    SELECT DATE(ct.fecha) fecha, rm.corte, rm.tipo_venta, rm.producto, com.descripcion, 
                    rm.precio precios,
                    COUNT(rm.id) ventas,
    ...
    if ($Desglose === "Cortes") {
        ...
        $selectByDia = "
            SELECT  CONCAT('| ',sub.fecha,' |') fecha ,sub.ventas ventas,sub.pesos pesos, sub.volumen volumen,SUM(VT.pesosA) pesos_ace,
            subN.pesos pesosN, subN.volumen volumenN,sub.descuento,subN.ventas ventasN, VT.ventasA
            FROM (
                ...
                SELECT rm.fecha, rm.corte, rm.ventas, 0 pesos, rm.volumenp volumen,rm.Producido,rm.ProducidoP
                FROM (
                    $selectRmDefaultN
                    GROUP BY DATE(ct.fecha)
                ) rm 
            ...
            LEFT JOIN (
                SELECT vt.corte,DATE( ct.fecha ) fecha, SUM( vt.total  ) pesosA, COUNT(1) ventasA
                FROM ct, man, vtaditivos vt
                WHERE 1 = 1 
                AND man.posicion = vt.posicion AND man.activo = 'Si'
                AND ct.id = vt.corte
        ...
        $selectByDia = "
            ...
            VT.pesosA pesos_ace,sub.descuento,subN.ventas ventasN, VT.ventasA
            FROM (
                SELECT  CONCAT('| ',rm.fecha,' |') fecha , rm.corte, rm.ventas, rm.pesosp pesos, rm.volumenp volumen,rm.descuento,rm.fecha fechajoin
                FROM (
                    $selectRmDefault
                    GROUP BY rm.fecha_venta
                ) rm 
            ...
            LEFT JOIN 
            (
                SELECT  CONCAT('| ',rm.fecha,' |') fecha, rm.corte, rm.ventas, 0 pesos, rm.volumenp volumen,rm.Producido,rm.ProducidoP
                FROM (
                    $selectRmDefaultN
                    GROUP BY rm.fecha_venta
                ) rm 
            ...
            LEFT JOIN (
                SELECT DATE( vt.fecha ) fecha, SUM( vt.total  ) pesosA, COUNT(1) ventasA
                FROM man, vtaditivos vt
                WHERE 1 = 1 
                AND man.posicion = vt.posicion AND man.activo = 'Si'
                AND DATE( vt.fecha ) BETWEEN DATE('$FechaI') AND DATE('$FechaF')
                ...
                AND vt.cantidad > 0
                    GROUP BY DATE( vt.fecha )
            ) VT ON DATE(sub.fechajoin) = DATE(VT.fecha) ";
    }
}
...
$selectByProducto = "
            SELECT rm.tipo_venta tipo, (rm.ventas) ventas, rm.descripcion producto, 
            rm.pesosp  pesos, rm.volumenp volumen,0 pesosN, 
...
$selectVF_AceitesFAC = "
            SELECT cli.tipodepago, fcd.producto, ROUND(SUM( fcd.cantidad ),2) cantidad, 
            'Venta facturada' descripcion, COUNT( * ) folios,
            ROUND(SUM((fcd.importe/(1+fcd.iva)) * fcd.iva),2) iva,
            ROUND(SUM(fcd.importe - ((fcd.importe/(1+fcd.iva)) * fcd.iva)),2) importe,
...
if ($Desglose == "Cortes") {
    ...
    $selectConcentradoAceGRAL = "
            SELECT vt.posicion,SUM(vt.cantidad) cantidad,SUM(vt.total) importe,iva
            FROM vtaditivos vt
            WHERE vt.tm = 'C'
            AND vt.cantidad > 0
            AND DATE(vt.fecha)  BETWEEN DATE('$FechaI') AND DATE('$FechaF')";
...
$Igualcortedia = utils\IConnection::execSql("SELECT valor FROM variables_corporativo WHERE llave='IgualCorteDia");
$Iespcorte = $Igualcortedia["valor"] != "1" ? "importe / precio" : "rm.volumenp";
$totaliesp = $Igualcortedia["valor"] != "1" ? "importe" : "rm.pesosp";
if (empty($Corte)) {
    if ($Desglose == "Cortes") {
        $selectIeps .= " SELECT 
        A.fecha,
    ...
    FROM
        ...
        (SELECT 
            ct.id corte,
                ...
                rm.iva,
                rm.precio,
                ROUND(SUM($totaliesp), 3) pesos,
                ROUND(SUM($Iespcorte), 3) volumen
        FROM
            ct
...
FROM (
    ...
    LEFT JOIN (
        SELECT DATE(rm.fin_venta) fecha,
            ...
            rm.iva,
            rm.precio,
            ROUND(SUM(rm.importe), 3) pesos,
            ROUND(SUM((importe / precio)), 3) volumen
        FROM rm
        WHERE DATE(rm.fin_venta) BETWEEN DATE('$FechaI') AND DATE('$FechaF')
            AND rm.pesos > 0 ";
...
ORDER BY rm.fecha ,rm.producto ASC;";
    ...
    $selectIeps = "
                SELECT rm.*,
                ROUND((rm.volumen * ( rm.precio -  rm.ieps))/(rm.iva + 1),4) importe,
                ROUND(rm.volumen * rm.ieps,2) iepsCuota,
                ROUND((rm.volumen * (rm.precio - rm.ieps)) - (rm.volumen * (rm.precio - rm.ieps))/(1 + rm.iva),3) iva,
                ...
                FROM(
                        ...
                        ROUND(SUM(rm.volumen),3) volumen, ROUND(SUM(rm.pesos),3) pesos, ROUND(SUM(rm.descuento)) descuento
                        FROM ct,rm 
...
$selectCFDI_Pagos = "
            SELECT * FROM (
                ...
                IFNULL(pagose.id_concat, '') pago, IFNULL(pagos.fecha_deposito, '') fecha_pago,  IFNULL(pagose.importe, 0) importe_pago
                FROM fc 
                LEFT JOIN cli ON fc.cliente = cli.id
                LEFT JOIN tt_pagos_por_factura pagose ON fc.id = pagose.factura
...
$selectVentaByCorteAbierto = "
            ...
            SELECT  man.isla_pos, m.dispensario,m.posicion,m.manguera,com.descripcion producto,com.clavei,descuento,
                    ...
                    (IFNULL(rmd.diferenciaL,0) + IFNULL(rmN.diferenciaL,0) + IFNULL(rmj.diferenciaL,0)) diferenciaDispensario,
                    (IFNULL(rmd.DiferenciaImporte,0) + IFNULL(rmN.DiferenciaImporte,0) + IFNULL(rmj.DiferenciaImporte,0)) diferenciaDispensarioImp
            ...
            (
                    ...
                    SUM(descuento) descuento,vendedor,ROUND(SUM(rm.volumen) - SUM(ROUND(rm.totalizadorvf - rm.totalizadorvi,3)),3) diferenciaL,
                    ROUND(SUM(pesos)-SUM(precio * ROUND(rm.totalizadorvf - rm.totalizadorvi,3)),2) DiferenciaImporte
            ...
            LEFT JOIN 
            (
                    SELECT man.isla_pos, rm.posicion,rm.manguera,rm.producto,COUNT(*) ventas,SUM(rm.pesos) pesos,
                    SUM(rm.volumen) volumen ,ROUND(SUM(volumen) - SUM(ROUND(totalizadorvf - totalizadorvi,3)),3) diferenciaL,
                    ROUND(SUM(pesos)-SUM(precio * ROUND(rm.totalizadorvf - rm.totalizadorvi,3)),2) DiferenciaImporte
                    FROM man, rm WHERE TRUE 
                    AND man.posicion = rm.posicion AND man.activo = 'Si'
                    AND rm.corte = $Corte AND rm.tipo_venta = 'N'
...
$selectVentaByCorteAbiertoDetalle = "
            ...
            IFNULL(mon.descuento,0) descuentoMon
            FROM man, man_pro m 
            LEFT JOIN 
            (
...
$selectTotales = "
            select * from (SELECT sub.cnt cargas, sub.producto descripcion,
            sub.incremento incremento,sub.neto as neto, SUM(volumenIngreso) volumenfac,SUM(volumenEgreso) volumenfacnc, SUM(importeIngreso) importeIngreso,SUM(importeEgreso) importeEgreso,
            ROUND(SUM(sub.importefac),2) importefac,  ROUND(SUM(sub.precioCompra),2) importeNet,sub.tipocarga
            FROM (
                SELECT 
                    c.id, c.producto
                    , DATE(me.fechae) fecha
                    , SUM(c.aumento) incremento
                    , SUM(c.tcaumento) as neto
                    ,COUNT(1) cnt
                    ,me.tipo tipocarga
                ...
                and date(c.fecha_insercion) BETWEEN DATE('$FechaI') AND DATE('$FechaF') GROUP BY c.producto,me.tipo
            ) sub GROUP BY sub.producto,sub.tipocarga
            ) ctr inner join (
...
$selectDescuento = "SELECT  rm.id folio,rm.corte,rm.posicion,com.descripcion,
	CONCAT('',rm.fin_venta) fin_venta,rm.volumen,rm.importe importe,
	rm.descuento descuento, cli.nombre,u.codigo codigo, 
 	IFNULL( GROUP_CONCAT(ven.alias), 'NO IMPRESO')despachador 
        FROM rm
        LEFT JOIN beneficios b ON rm.id=b.id_consumo 
        LEFT JOIN unidades u ON u.id=b.id_unidad
        LEFT JOIN cli ON cli.id=u.cliente
        left join com ON rm.producto = com.clavei
        inner join impresiones_pos ON rm.id = impresiones_pos.id_rm 
		left join ven ON impresiones_pos.id_ven = ven.id
    WHERE rm.descuento > 0 
    AND rm.fecha_venta like '%" . str_replace("-", "", date('Ymd', strtotime($FechaHI))) . "%'
    AND rm.fecha_venta like '%" . str_replace("-", "", date('Ymd', strtotime($FechaHF))) . "%'
    AND rm.fin_venta BETWEEN '$FechaHI' AND '$FechaHF' 
    AND b.tipo='I'
    GROUP BY rm.id
	ORDER BY rm.corte,rm.posicion,rm.fin_venta DESC;";

$SelectFPG = "
    Select fc.id,fc.serie,fc.folio,cli.rfc,cli.nombre,fc.fecha,
    (fc.importe + fc.ieps) importe,fc.descuento,fc.iva,fc.total,
    CASE 
        WHEN fc.status = 0 THEN 'Abierto'
        WHEN fc.status = 1 THEN 'Timbrado'
        WHEN fc.status = 2 THEN 'En proceso'
        WHEN fc.status = 1 THEN 'Cancelado'
        ELSE 'Sin registro'
    END AS status,
    CASE 
            WHEN LOCATE('Contado', fc.observaciones ) > 0 THEN 'Contado'
            WHEN LOCATE('Monedero', fc.observaciones ) > 0 THEN 'Monedero'
            WHEN LOCATE('Tarjeta', fc.observaciones) > 0 THEN 'Tarjeta'
            WHEN LOCATE('Aceites', fc.observaciones) > 0 THEN 'Aditivos'
            ELSE 'No encontrado'
        END AS tipo,
        fc.usr,'Factura' factura
        from fc 
            inner join cli on fc.cliente = cli.id 
                where cli.rfc = 'XAXX010101000' and nombre like '%PUBLICO%' 
                and fecha_indice BETWEEN " . str_replace("-", "", $FechaI) . " AND " . str_replace("-", "", $FechaF) . " ";

if ($FormaPago !== "*") {

    $SelectFPG .= "HAVING tipo = '" . $FormaPago . "'";
}

$SqlAdd = "";
if ($SCliente > 0) {
    $IdCli = explode("|", $SCliente);
    $SqlAdd = "AND cli.id = " . $IdCli[0];
}
$SelectTraslados = "SELECT i.id,cli.nombre, cli.tipodepago ,CONCAT(i.serie ,' - ', i.folio) folio, uuid ,
    i.fecha,id.importe, id.iva,id.ieps retencion,id.precio, (id.precio * id.iva) importeIva, 
    ((id.precio + (id.precio * id.iva)) - id.importe) retencion FROM omicrom.ingresos_detalle id
    LEFT JOIN ingresos i ON i.id=id.id LEFT JOIN cli ON cli.id = i.cli
    WHERE producto = 0 AND i.uuid != '-----' AND DATE(fecha) BETWEEN DATE('$FechaI') AND DATE('$FechaF') $SqlAdd ORDER BY cli.id, i.fecha;";
