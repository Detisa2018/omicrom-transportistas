...
//                    if ($fcResult["total"] >= $pagoVO->getImporte()) {
                    $Sql = "
                            ...
                            FROM pagos p, fc  WHERE p.id = fc.relacioncfdi AND p.id = '$cVarVal' AND fc.status = 1";
//                    } else {
//                        $Sql = "
//                            INSERT INTO nc ( fecha, cliente, cantidad, importe, iva, ieps, total, observaciones, formadepago, metododepago, factura, usocfdi, tiporelacion, relacioncfdi, usr)
...
//                    }
            ...
            try {
                ...
                $Cliente = "SELECT cli.id FROM pagos p LEFT JOIN fc ON " . $request->getAttribute("IdFc") . " = fc.id "
                        . " LEFT JOIN cli ON cli.id = p.cliente "
                        . "WHERE p.id = '$cVarVal'";
                $idCli = utils\IConnection::execSql($Cliente);
                $ValidaExistencia = "SELECT id_nc id FROM relacion_cfdi WHERE "
                        . "uuid = (SELECT uuid FROM fc WHERE id = " . $request->getAttribute("IdFc") . ") AND uuid_relacionado = '" . $pagoVO->getUuid() . "';";

                $rsNc = utils\IConnection::execSql($ValidaExistencia);
                if (!($rsNc["id"] > 0)) {
                    $Sql = "INSERT INTO nc ( fecha, cliente, cantidad, importe, iva, ieps, total, observaciones, formadepago, metododepago, factura, usocfdi, tiporelacion, relacioncfdi, usr)
                            ...
                            '30' formapago , 'PUE' metododepago , p.id factura ,IF(cli.rfc='XAXX010101000','S01',$UsoCfdi) , '07' tiporelacion, 
                            '" . $request->getAttribute("IdFc") . "' relacioncfdi,
                            '" . $usuarioSesion->getUsername() . "' usr
                            FROM pagos p LEFT JOIN fc ON " . $request->getAttribute("IdFc") . " = fc.id "
                            . " LEFT JOIN cli ON cli.id = p.cliente "
                            . "WHERE p.id = '$cVarVal'";
                    error_log($Sql);
                    if (!$mysqli->query($Sql)) {
                    ...
                    $UpdateRelacionCfdi = "UPDATE relacion_cfdi SET id_nc = " . $mysqli->insert_id . " WHERE "
                            . "uuid = (SELECT uuid FROM fc WHERE id = " . $request->getAttribute("IdFc") . ") AND uuid_relacionado = '" . $pagoVO->getUuid() . "';";
                    utils\IConnection::execSql($UpdateRelacionCfdi);
                    // Recuperamos el max id de nc
                    $SqlNc = " SELECT id,relacioncfdi FROM nc WHERE formadepago = '30' AND factura = '$cVarVal' AND status = " . StatusNotaCredito::ABIERTO . " ";
                    error_log($SqlNc);
                    $queryNc = $mysqli->query($SqlNc);
                    ...
                    $UpdateNc = "UPDATE nc SET fecha = '$DateListo' WHERE id = " . $rsNc["id"];
                    utils\IConnection::execSql($UpdateNc);
                }
                error_log("Retrive id nc: " . $rsNc['id']);
