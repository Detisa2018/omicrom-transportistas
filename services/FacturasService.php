    ...
    try {
        ...
        if ($request->getAttribute("op") === "Asentar") {
            ...
            if ($clienteVO->getTipodepago() === TiposCliente::CREDITO || $clienteVO->getTipodepago() === TiposCliente::PREPAGO) {
                ...
                $sqlInsertTickets = "INSERT INTO cxc (cliente,referencia,fecha,hora,tm,concepto,cantidad,importe,corte,producto,factura)
                                    SELECT fc.cliente,fcd.ticket referencia,IFNULL(DATE(rm.fin_venta),date(fc.fecha)) fecha,IFNULL(TIME(rm.fin_venta),CURTIME()) hora,'C' tm,
                                    IFNULL(IF(fcd.producto <= 4,'Venta de combustible',vtaditivos.descripcion),'Aditivo manual') concepto,fcd.cantidad,
                                    fcd.importe importe ,IFNULL(IF(fcd.producto <= 4,rm.corte,vtaditivos.corte),(SELECT corte FROM omicrom.islas WHERE isla = 1)) corte,IF(fcd.producto <= 4,rm.producto,'-') producto,fc.id factura 
                                    FROM fc,fcd
