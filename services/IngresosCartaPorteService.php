...
try {
    if ($request->getAttribute("Comprobante") === "Nuevo") {
        ...
        $CpVO->setTabla("Ingresos");
        if ($idcp = $CpDAO->create($CpVO)) {
            $ingresosVO = $ingresosDAO->retrieve($busca);

            $SerTraDAO = new ServiciosTraDAO();
            $SerTraVO = new ServiciosTraVO();
            $SerTraVO = $SerTraDAO->retrieve($request->getAttribute("DescripcionServicio"));

            $ingresosVO->setId_prv($request->getAttribute("TerminalAlmacenamiento"));
            $ingresosVO->setId_servicios_tra($request->getAttribute("DescripcionServicio"));
            $ingresosVO->setClaveProdServ($SerTraVO->getClave_producto());
            $CostoServicio = $SerTraVO->getPrecio();
            $ingresosDAO->update($ingresosVO);
            $Precio = $CostoServicio / 1.16;
            ...
            $ingresosDetalleVO->setImporte($CostoServicio);
            $ingresosDetalleVO->setPreciob($CostoServicio);
            $ingresosDetalleVO->setId_rm(0);
        ...
        $ingresosDAO->update($ingresosVO);
    } else if ($request->getAttribute("Boton") === "Agregar Producto") {
        ...
        $Inv = utils\IConnection::execSql($cSQL);
        if ($request->getAttribute("PrecioTotal") > 0) {
            $PrecioxLitro = $request->getAttribute("PrecioTotal") / $request->getAttribute("Cantidad");
        } else {
            $PrecioxLitro = $request->getAttribute("PrecioxLitro");
        }
        $Iva = $Inv['iva'];
        $Ieps = $Inv['ieps'];
        $PrecioU = ($PrecioxLitro == 0) ? 0 : ROUND(($PrecioxLitro - $Ieps) / (1 + $Iva), 3);
        ...
        $ingresosDetalleDAO = new Ingresos_detalleDAO();
        $ingresososdDetalleVO->setId($busca);
        ...
        if (($id = $ingresosDetalleDAO->create($ingresososdDetalleVO)) > 0) {
            TotalizaCartaPorte($busca);
            $Msj = utils\Messages::RESPONSE_VALID_CREATE;
        } else {
            $Msj = utils\Messages::RESPONSE_ERROR;
        }
    } else if ($request->getAttribute("Boton") === utils\Messages::OP_UPDATE) {
        $IngresoVO = new IngresosVO();
        $IngresoDAO = new IngresosDAO();
        error_log("Mostramos Busca " . $busca);
        $IngresoVO = $IngresoDAO->retrieve($busca);
        $IngresoVO->setUsocfdi($request->getAttribute("cuso"));
        $IngresoVO->setFecha($request->getAttribute("Fecha"));
        $IngresoVO->setFormadepago($request->getAttribute("Formadepago"));
        $IngresoVO->setMetodopago($request->getAttribute("Metododepago"));
        $IngresoDAO->update($IngresoVO);

        $CliDAO = new ClientesDAO();
        $CliVO = new ClientesVO();
        $CliVO = $CliDAO->retrieve($IngresoVO->getId_cli());
        $CliVO->setRfc($request->getAttribute("Rfc"));
        $CliVO->setCodigo($request->getAttribute("CodigoPostal"));
        $CliVO->setNombre($request->getAttribute("Nombre"));
        $CliVO->setRegimenFiscal($request->getAttribute("RegimenFiscal"));
        $CliVO->setCorreo($request->getAttribute("CorreoElectronico"));
        $CliVO->setEnviarcorreo($request->getAttribute("Enviarcorreo") === "Si" ? "Si" : "No");
        $CliDAO->update($CliVO);
    }
} catch (Exception $ex) {
...
if ($request->getAttribute("Op") === "Clean") {
    ...
    error_log("MOSTRAMOSSSSS QUE RESPONDE ");
    ...
    $SqlCli = "SELECT enviarcorreo, deencrypt_data(cli.correo) correo FROM ingresos i LEFT JOIN cli ON cli.id = i.id_cli WHERE i.id = $busca";
    $Cc = utils\IConnection::execSql($SqlCli);
    if ($Cc["enviarcorreo"] === "Si") {
        $Msj = enviarCorreo($result["return"]["uuid"], $Cc["correo"], "");
    }
...
} elseif ($request->getAttribute("op") === "EliminaDetalle") {
    $DeleteDt = "DELETE FROM ingresos_detalle WHERE idnvo = " . $request->getAttribute("Idnvo");
    utils\IConnection::execSql($DeleteDt);
    TotalizaCartaPorte($busca);
    header("Location: IngresosCartaPorte.php?Msj" . utils\Messages::RESPONSE_VALID_DELETE);
}
