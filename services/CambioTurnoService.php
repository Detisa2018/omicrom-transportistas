    ...
    try {
        if ($request->getAttribute("Boton") === utils\Messages::OP_ADD) {
            ...
            if ($ConcentrarVtasTarjeta === ConcentrarTarjetasCorte::SI && $request->hasAttribute("Vendedor")) {
                $Msj = concentrarTarjetasEnCorte($sanitize->sanitizeFloat("Importe"), $sanitize->sanitizeInt("Vendedor"), $sanitize->sanitizeString("Vc"), $Cliente, $Corte, $mysqli);
            } elseif ($Ticket > 0 && $request->hasAttribute("Dolares")) {
                $Msj = relacionaVentaEnDolares($Ticket, $sanitize->sanitizeFloat("Dolares"), $rmDAO, $mysqli);
            } elseif ($Ticket > 0 && !$request->hasAttribute("Unidad")) {
                $Msj = agregarTicketACliente($Ticket, $Corte, $sanitize->sanitizeFloat("Pagoreal"), $machaca, $sanitize->sanitizeString("Placas"), $sanitize->sanitizeString("Vdm"), $sanitize->sanitizeString("Kilometraje"), $Cliente, $usuarioSesion, $ctVO);
            } elseif ($Ticket > 0 && $request->hasAttribute("Unidad")) {
                $SelectCodigo = utils\IConnection::execSql("SELECT CONCAT(id,' | ', codigo) codigo FROM unidades WHERE id = " . $sanitize->sanitizeString("Unidad"));
                $Msj = relacionPorUnidad($Cliente, $SelectCodigo["codigo"], $Ticket, $sanitize->sanitizeString("Kilometraje"), $sanitize->sanitizeString("Placas"));
            } elseif ($request->hasAttribute("Concepto") && $request->hasAttribute("Importe")) {
                $Msj = addInCtpagos($Corte, $sanitize->sanitizeString("Concepto"), $sanitize->sanitizeFloat("Importe"), $mysqli);
            } elseif ($request->hasAttribute("Banco") && $request->hasAttribute("Importe")) {
                $bancoVO = $bancosDAO->retrieve($sanitize->sanitizeInt("Banco"));
                $Msj = addRegistroaBancos($Corte, $bancoVO, $sanitize->sanitizeFloat("Importe"), $sanitize->sanitizeString("Plomo"), $sanitize->sanitizeString("Descripcion"), $BitacoraDAO, $usuarioSesion, $ctVO);
            } else if ($request->getAttribute("AddTicketContado") === "ok") {
                $Msj = insertaTicketCliEfectivo($request->getAttribute("TicketEfectivo"), $request->getAttribute("ClienteEfectivo"));
            }
    ...
    try {
        if ($request->getAttribute("op") === utils\Messages::OP_DELETE) {
            if ($Tipo === RmDAO::TIPO) {
                ...
                $clienteVO = $clienteDAO->retrieve($rmVO->getCliente());
                $Codigo = $rmVO->getCodigo();
                $IdCliente = $rmVO->getCliente();
                if ($ConcentrarVtasTarjeta === ConcentrarTarjetasCorte::NO || (is_numeric($rmVO->getId()) && $rmVO->getCorte() == $Corte)) {
                    ...
                    if ($rmVO->getUuid() !== FcDAO::SIN_TIMBRAR && ($clienteVO->getTipodepago() === TiposCliente::CREDITO || $clienteVO->getTipodepago() === TiposCliente::PREPAGO)) {
                        ...
                        if ($rmDAO->update($rmVO)) {
                            ...
                            regresaSaldoSiEsUnidadSaldos($Codigo, $rmVO->getImporte(), $IdCliente, $rmVO->getId(), $usuarioSesion->getNombre());
                        } else {
                            $Msj = utils\Messages::RESPONSE_ERROR;
                        }
                    }
...
function concentrarTarjetasEnCorte($importe, $vendedor, $vaucher, $Cliente, $Corte, $mysqli) {
    $rmDAO = new RmDAO();
    $Ticket = insertInCtTarjatas($Corte, $Cliente, $importe, $vendedor, $vaucher, $mysqli);
    if ($Ticket > 0 && $Ticket !== false) {
        deleteDataInCxc($Ticket, $Corte, $mysqli);
        $Msj = insertIntoCxc($Ticket, $Cliente, $Corte, $vendedor, $importe, $rmDAO, $mysqli);
    }
    return $Msj;
}

function insertInCtTarjatas($Corte, $Cliente, $importe, $vendedor, $vaucher, $mysqli) {
    $sql = "INSERT INTO cttarjetas (id,banco,importe,vendedor,vaucher) VALUES (? ,? ,? ,? ,? )";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("sssss", $Corte, $Cliente, $importe, $vendedor, $vaucher);
    if ($stmt->execute()) {
...
function deleteDataInCxc($Ticket, $Corte, $mysqli) {
    $sql = "UPDATE cxc SET cliente = -cliente, referencia = -referencia, corte = -corte "
            . "WHERE referencia=? AND tm='C' AND corte = ? LIMIT 1";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("ii", $Ticket, $Corte);
    if ($stmt->execute()) {
        return true;
    }
}

function insertIntoCxc($Ticket, $Cliente, $Corte, $vendedor, $importe, $rmDAO, $mysqli) {
    $rmVO = new RmVO();

    $rmVO = $rmDAO->retrieve($Ticket);
    $create = "INSERT INTO cxc (cliente,placas,referencia,fecha,hora,tm,concepto,cantidad,importe,recibo,corte,producto,rubro,factura,fechaHoraRegistro) VALUES "
            . "('$Cliente','','$Ticket','" . date("Y-m-d", strtotime($rmVO->getFin_venta())) . "','" . date("H:i:s", strtotime($rmVO->getFin_venta())) . "','C',"
            . "'Venta del corte $Corte Vendedor: $vendedor','1','$importe','0','$Corte','-','-----','0','" . date("Y-m-d H:i:s") . "');";
...
function relacionaVentaEnDolares($Ticket, $dolares, $rmDAO, $mysqli) {
    $rmVO = $rmDAO->retrieve($Ticket);
    $divisas = getTipoDeCambio($mysqli);
    if (is_array($divisas) && $divisas["tipo_de_cambio"] > 0) {
        $real = $dolares * $divisas["tipo_de_cambio"];
        EliminaRegistrosExistentes($mysqli, $Ticket);
        if ($real == $rmVO->getPesos()) {
            $result = $rmVO->getPesos() / $divisas["tipo_de_cambio"];
        } else {
            $result = $real / $divisas["tipo_de_cambio"];
        }
        $Msj = insertaFormasDePago($Ticket, $divisas["tipo_de_cambio"], $result, $mysqli);
    } else {
...
function getTipoDeCambio($mysqli) {
    $selectDivisa = "SELECT tipo_de_cambio FROM divisas WHERE clave = 'USD' LIMIT 1;";
    return $mysqli->query($selectDivisa)->fetch_array();
}

function EliminaRegistrosExistentes($mysqli, $Ticket) {
...
function insertaFormasDePago($Ticket, $TipoDeCambio, $result, $mysqli) {
    $insertFormas = "INSERT INTO formas_de_pago VALUES ('$Ticket','01','USD','" . $TipoDeCambio . "','$result')";
...
function validaDescuentoACliente($Ticket, $Cliente, $pagoReal) {
    $VariablesCorp = "SELECT valor FROM variables_corporativo WHERE llave = 'descuento_por_cli' ";
    $Vvl = utils\IConnection::execSql($VariablesCorp);
    if ($Vvl["valor"] == 1) {
        $Cli = "SELECT descuentocli FROM cli WHERE id = $Cliente";
        $Desc = utils\IConnection::execSql($Cli);
        if ($Desc["descuentocli"] > 0) {
            $RmT = "SELECT importe FROM rm WHERE id = $Ticket";
            $Rm = utils\IConnection::execSql($RmT);
            $pagoReal = ($Rm["importe"] * (1 - ($Desc["descuentocli"] / 100)));
            return $pagoReal;
        } else {
            return $pagoReal;
        }
...
function agregarTicketACliente($Ticket, $Corte, $pagoReal, $machaca, $Placas, $Vdm, $Kilometraje, $Cliente, $usuarioSesion, $ctVO) {
    $rmDAO = new RmDAO();
    $clienteDAO = new ClientesDAO();
    $cxcDAO = new CxcDAO();
    $pagoReal = validaDescuentoACliente($Ticket, $Cliente, $pagoReal);

    if (validaTicketSinProcesos($Ticket)) {
            ...
            if (!is_null($clienteVO) && $clienteVO->getId() > 0 && $clienteVO->getActivo() === "Si") {
                if (is_numeric($pagoReal) && $pagoReal > 0 && ($pagoReal - $machaca) <= $rmVO->getImporte() && (($pagoReal - $machaca) <= $rmVO->getImporte() || $clienteVO->getTipodepago() === TiposCliente::TARJETA || $clienteVO->getTipodepago() === TiposCliente::VALES || $clienteVO->getTipodepago() === TiposCliente::MONEDERO)) {
                    if ($ctVO->getStatusctv() === StatusCorte::ABIERTO) {
                        ...
                        $Placas = empty($Placas) ? $rmVO->getPlacas() : $Placas;
                        $Vdm = empty($Vdm) ? $rmVO->getVdm() : $Vdm;
                        ...
                        $rmVO->setEnviado(0);
                        $clienteVO->getTipodepago() === "Consignacion" ? $rmVO->setTipo_venta("N") : "";
                        ...
                        if ($rmDAO->update($rmVO)) {
                            ...
                            CancelaLosCxc($Ticket, $rmVO->getProducto());
...
function validaTicketSinProcesos($Ticket) {
    ...
    return $Tper["prm"] == 1 ? true : false;
}

function validaTicketyCliente($pagoReal, $machaca, $rmVO) {
    error_log("Validando ticket y clienteF");
    if (is_numeric($pagoReal) && $pagoReal > 0 && ($pagoReal - $machaca) <= $rmVO->getImporte() && (($pagoReal - $machaca) <= $rmVO->getImporte() || $clienteVO->getTipodepago() === TiposCliente::TARJETA || $clienteVO->getTipodepago() === TiposCliente::VALES || $clienteVO->getTipodepago() === TiposCliente::MONEDERO)) {
        return true;
    } else {
        return false;
...
function CancelaLosCxc($Ticket, $Producto) {
    $updateCxc = "UPDATE cxc SET cliente = -cliente, referencia = -referencia, corte = -corte "
            . "WHERE referencia='$Ticket' AND tm='C' AND producto = '" . $Producto . "' LIMIT 1";
    utils\IConnection::execSql($updateCxc);
}

function insertaTicketCliEfectivo($TicketEfectivo, $ClienteEfectivo) {
    $Tkt = utils\IConnection::execSql("SELECT id FROM rm WHERE id = '" . $TicketEfectivo . "' AND cliente = 0 ");
    if (is_numeric($Tkt["id"])) {
        $IdCli = explode("|", $ClienteEfectivo);
        $UpdateTicket = "UPDATE rm SET cliente = '" . $IdCli[0] . "' WHERE id = '" . $Tkt["id"] . "';";
        $Rss = utils\IConnection::execSql($UpdateTicket);
        $Msj = utils\Messages::RESPONSE_VALID_UPDATE;
    } else {
...
function addInCtpagos($Corte, $concepto, $importe, $mysqli) {
...
function addRegistroaBancos($Corte, $bancoVO, $importe, $plomo, $descripcion, $BitacoraDAO, $usuarioSesion, $ctVO) {
    $BitacoraDAO->saveLog($usuarioSesion->getUsername(), "ADM", "Agrega abono a bancos corte $Corte e importe : " . $importe);
    if ($bancoVO->getTipo_cambio() != 1) {
        $Importe = $importe * $bancoVO->getTipo_cambio();
    } else {
        $Importe = $importe;
    }
    $concepto = strtoupper(trim($descripcion));
...
function regresaSaldoSiEsUnidadSaldos($Codigo, $Importe, $Cliente, $Ticket, $NameSession) {
    $Sql = "SELECT  periodo,importe,id FROM unidades WHERE codigo = '$Codigo'";
    $Periodo = utils\IConnection::execSql($Sql);
    if ($Periodo["periodo"] === "B") {
        $UpdateTarjeta = "UPDATE unidades SET importe = importe + " . $Importe . " WHERE cliente = " . $Cliente
                . " AND periodo = 'B' AND impreso = '" . $Codigo . "'";
        utils\IConnection::execSql($UpdateTarjeta);
        $Suma = $Periodo["importe"] + $Importe;
        $Insrt = "INSERT INTO unidades_log (noPago,importeAnt,importe,importeDelPago,idUnidad,usr)  VALUES ($Ticket,'" . $Periodo["importe"] . "','" . $Suma . "','$Importe','" . $Periodo["id"] . "',' $NameSession');";
