...
error_log ('La variable orden es '.$orden);

$selectCxp = "";

if ($orden == "factura") {
    $selectCxp .= " 
    SELECT referencia, fecha, fechav, concepto, cargo, abono
FROM (

    SELECT cxp.referencia, 
           cxp.fecha, 
           cxp.fechav,
		   concat('Pago de facturas ',replace(group_concat(REGEXP_REPLACE( cxp.concepto , '[^0-9]', '')),cxp.numpago,'') ) AS concepto,
           0 AS cargo,
           SUM(cxp.importe) AS abono,
           cxp.numpago
    FROM $Tabla AS cxp   
    WHERE cxp.proveedor = '$Proveedor'
      AND cxp.fecha BETWEEN DATE('$FechaI') AND DATE('$FechaF')
      AND cxp.tm = 'H'
    GROUP BY cxp.numpago
    ...
    SELECT cxp.referencia, 
           cxp.fecha, 
           cxp.fechav,
           IF(cxp.tm = 'C', CONCAT(cxp.concepto, ' Compra Num: ', cxp.referencia,' Folio Fac. ',me.foliofac ), cxp.concepto) AS concepto,
           cxp.importe AS cargo,
           0 AS abono,
           cxp.numpago
    FROM $Tabla AS cxp  
    inner join me on cxp.referencia = me.id
    WHERE cxp.proveedor = '$Proveedor' 
      AND cxp.fecha BETWEEN DATE('$FechaI') AND DATE('$FechaF') 
      AND cxp.tm = 'C'
) AS resultados
ORDER BY fecha, numpago, referencia
...
error_log('La factura es: '.$selectCxp);
