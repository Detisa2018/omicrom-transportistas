...
if ($request->hasAttribute("Boton") && $request->getAttribute("Boton") !== utils\Messages::OP_NO_OPERATION_VALID) {
    ...
    try {
        ...
        if ($request->getAttribute("Boton") === "Agregar ticket") {
            ...
            error_log("ITEM " . $FilterStatement);
                ...
                if ($Pass || $VC["valor"] != 1 || $clienteVO->getTipodepago() !== "Contado") {
                    if ($array['fcd'] == FALSE && $array['siCxc'] == TRUE) {
                        ...
                        if ($rmVO->getTipodepago() === "Consignacion") {
                            ...
                            $SqlComisiones = "SELECT * FROM comisiones WHERE vigencia >= '" . $DateComision . "';";
                            ...
                            $MontoConIva = $RsCom["monto"] * (1 + $rmVO->getIva());
                                        ...
                                        while ($Item = $Items->fetch_array()) {
                                            if ($Item["tipoMv"] === "Pasa") {
                                                if (($clienteVO->getTipodepago() != TiposCliente::CREDITO && $clienteVO->getTipodepago() != TiposCliente::PREPAGO) || $Item['cxc'] != NULL || validaTicketLibre($clienteVO)) {
                                                    ...
                                                    $Msj = "Error: El ticket " . $Ticket . " no está cargado en el estado de cuenta del cliente.";
                                                }
                                            } else {
                                                $Msj = "Error: El ticket ya fue impreso y procesado, Favor de comunicarse con soporte";
                                            }
                                        }
                                    } else {
...
// En tipo 'cantidad' el valor del importe es calculado
            ...
            if ($request->getAttribute("AddTickets") === "BuscaTicket") {
                if ($Tipo === "I") {
                    ...
                    $Concat = " AND (rm.importe/rm.precio) <= " . $request->getAttribute("Cantidad") . " ";
                }

                $SelectTicketDisp = "SELECT rm.id,rm.ieps,rm.producto,rm.importe pesos,rm.tipo_venta,rm.fin_venta,rm.producto,(rm.importe/rm.precio) volumen,rm.precio,inv.id idInv "
                        . "FROM rm "
                        ...
                        if ($Tipo === "I") {
                            ...
                            if ($TotalaEliminar >= $rgs["pesos"] || ($TotalaEliminar > -100 && $TotalaEliminar <= 3 && $rgs["pesos"] <= $Restante)) {
                                if ($fcdDAO->create($fcdVO)) {
                                    BitacoraDAO::getInstance()->saveLog($usuarioSesion->getNombre(), "ADM", "Relaciona ticket por medio de busqueda " . $rgs["id"]);
                    ...
                    if (($Importe > 0 && $PrecioU > 0) || ($request->getAttribute("Producto") > 4)) {
                        ...
                        if (($id = $fcdDAO->create($fcdVO)) > 0) {
                            if ($sanitize->sanitizeString("EdoCuenta") === "Si") {
                                ...
                                $cxcDAO = new CxcDAO();
                                $cxcDAO->create($cxcVO);
                                $CntRetencion = "SELECT retiene_iva,porcentaje FROM inv where id = " . $fcdVO->getProducto();
                                $Retencion = utils\IConnection::execSql($CntRetencion);
                                if ($Retencion["retiene_iva"] === "Si") {
                                    error_log("ENTRAMOS A RETENCION");
                                    $IvaCia = utils\IConnection::execSql("SELECT iva/100 iva_C, iva FROM cia;");
                                    $IvaTotal = $Importe * $IvaCia["iva_C"];
                                    $PorcentajeIva1 = $IvaTotal / $IvaCia["iva"];
                                    $Retencion = ($PorcentajeIva1 * $Retencion["porcentaje"]) * -1;
                                    $cxcVO->setCantidad(1);
                                    $cxcVO->setConcepto("Retencion IVA");
                                    $cxcVO->setImporte($Retencion);
                                    $cxcDAO->create($cxcVO);
                                }
                            }
        ...
        if ($request->getAttribute('action') === "devolucion") {
            ...
            if ($fcdVO->getTicket() > 0) {
                ...
                $sql = "UPDATE cxc SET cliente = -cliente, referencia = - referencia, cantidad = -cantidad,"
                        ...
                        . "AND cliente = '" . $fcVO->getCliente() . "' AND (concepto LIKE '%manual%' || concepto = 'Retencion IVA') ";
...
function getRMItems($filterRM, $filterAdt, $tipoDePago) {
                    ...
                    SELECT
                        ...
                        FROM(
                            SELECT
                                   ...
                                   IF('$tipoDePago' = 'Contado' AND rm.comprobante > 0 AND rm.importe  < rm.pesos,'No Pasa','Pasa') tipoMv, 
                                   rm.iva,
    ...
    if ($tipoDePago === TiposCliente::CREDITO || $tipoDePago === TiposCliente::PREPAGO) {
        $cSQL .= " LEFT JOIN cxc ON cxc.referencia = TOT.id AND cxc.tm = 'C' AND cxc.concepto NOT LIKE '%Producto manual%' AND cxc.cliente = TOT.cliente";
    }
