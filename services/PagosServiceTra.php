...
if ($request->hasAttribute("id")) {
    $returnLink = urlencode("pagosTrae.php?");
    $backLink = urlencode("pagosTra.php?criteria=ini");
...
if ($request->hasAttribute("Boton") && $request->getAttribute("Boton") !== utils\Messages::OP_NO_OPERATION_VALID) {
    ...
    try {
        ...
        if ($request->getAttribute("Boton") === utils\Messages::OP_ADD) {
            ...
            $Msj = procesoInsertaPagos($pagoVO, $clienteVO);
            if ($request->getAttribute("CantidadPagos") > 1) {
                $CntRegistros = $request->getAttribute("CantidadPagos") - 1;
                for ($e = 1; $e <= $request->getAttribute("CantidadPagos"); $e++) {
                    if ($e > 1) {
                        $pagoVO->setImporte($sanitize->sanitizeFloat("ImporteNo" . $e));
                        $pagoVO->setConcepto($sanitize->sanitizeString("ConceptoNo" . $e));
                        $Msj = procesoInsertaPagos($pagoVO, $clienteVO);
                    }
                }
            }
...
function procesoInsertaPagos($pagoVO, $clienteVO) {
    $pagoDAO = new PagoDAO();
    if (($id = $pagoDAO->create($pagoVO)) > 0) {
        actualizaSerie($id, obtenSerie($clienteVO->getTipodepago()));

        $Return = "pagosTrad.php?criteria=ini&cVarVal=" . $id;
        $islaDAO = new IslaDAO();
        $islaVO = new IslaVO();
        ...
        $variablesCorpDAO = new V_CorporativoDAO();
        $variablesCorpVO = new V_CorporativoVO();
...
function obtenSerie($Tipodepago) {
...
function actualizaSerie($id, $Serie) {
    $Upd = "UPDATE pagos SET serie = '" . $Serie . "' WHERE id=" . $id;
