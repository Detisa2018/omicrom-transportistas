...
if ($orden == "factura") {
    ...
    SUB.placas,
    SUB.fecha,
    DATEDIFF(CURRENT_DATE(), SUB.fecha) antiguedad,
    IF(ISNULL(SUB.factura) OR SUB.factura = 0,
        CONCAT(SUB.conceptoOriginal, ' ', descripcion),
        conceptoFactura) concepto,
    SUB.factura,
    IF(SUB.tm = 'C', SUB.importe, 0) cargo,
    IF(SUB.tm = 'H', SUB.importe, 0) abono
FROM
    (SELECT 
            cxc.fecha,
            IF(ISNULL(cxc.factura) OR cxc.factura = 0, cxc.referencia, cxc.factura) referencia,
            cxc.tm,
            cxc.placas,
            IFNULL(CASE cxc.factura
                WHEN (ISNULL(cxc.factura) OR cxc.factura = 0) THEN cxc.concepto
                ELSE CONCAT(IF(cxc.tm = 'H', 'Pago De ', ''), 'Factura No. ', IF(cxc.factura = 0, cxc.referencia, CONCAT(IF(ISNULL(fc.serie) OR fc.serie = '', '', CONCAT(fc.serie, '-')), fc.folio)))
            END, cxc.concepto) conceptoFactura,
            IFNULL(com.descripcion, '') descripcion,
            cxc.concepto conceptoOriginal,
            ROUND(SUM(cxc.importe), 2) importe,
            CONCAT(IF(ISNULL(fc.serie) OR fc.serie = '', '', CONCAT(fc.serie, '-')), fc.folio) factura,
    ...
    LEFT JOIN fc ON cxc.factura = fc.id
    LEFT JOIN com ON com.clavei = cxc.producto
    inner join ct on cxc.corte = ct.id
    WHERE
        cxc.cliente = '$Cliente'
            AND date(ct.fecha) BETWEEN DATE('$FechaI') AND DATE('$FechaF') 
            AND cxc.cliente > 0
    GROUP BY IFNULL(IF(cxc.factura = 0, cxc.referencia, cxc.factura), cxc.referencia) , cxc.tm
    ORDER BY cxc.factura , cxc.fecha , cxc.tm) SUB
...
if ($Desglose === "Cortes") {
    if (!empty($Turno) && $Turno !== "*") {
        $cWhere = " AND ct.turno = " . $Turno;
    }
    $cWhere .= " AND DATE( ct.fecha ) BETWEEN DATE( '$FechaI' ) AND DATE( '$FechaF' )";
    ...
    $cWhere .= " AND rm.fecha_venta BETWEEN CAST( DATE_FORMAT( DATE( '$FechaI' ), '%Y%m%d' ) AS UNSIGNED ) AND CAST( DATE_FORMAT( DATE( '$FechaF' ), '%Y%m%d' ) AS UNSIGNED )";
    $cWhereT .= " AND cttarjetas.fecha BETWEEN DATE( '$FechaIQ' ) AND DATE( '$FechaFQ' )"; //cttarjetas
    $cWhereA .= " AND vt.fecha BETWEEN ( '$FechaIQ' ) AND ( '$FechaFQ' )"; //aditivos
}

if (!empty($Cliente)) {
    $cWhere .= " AND rm.cliente = " . $Cliente;
    $cWhereT .= " AND cttarjetas.banco = '$Cliente'";
    $cWhereA .= " AND vt.cliente = '$Cliente'";
}
...
if ($TipoCliente !== "*") {
    ...
    $cWhereTC .= " AND cli.tipodepago = '$TipoCliente'";
    $cWhereBP .= " AND cli.tipodepago = '$TipoCliente'";
}
...
SELECT 
	'0' tipo, 
	cli.tipodepago, rm.cliente, cli.nombre,
	man.isla_pos, rm.corte, rm.id ticket, rm.volumen, rm.pesos importe, rm.pagoreal, rm.iva, rm.ieps,
	'' descripcion, com.descripcion producto, rm.fin_venta fecha, rm.uuid, rm.kilometraje, 
        IF( rm.codigo REGEXP '([0-9]{10,13}[|]?)+', 'Vales', rm.codigo ) codigo, 
        IFNULL( unidades.impreso, '-----' ) impreso, IFNULL( unidades.numeco, '-----' ) numeco,
        IF(rm.placas != '', rm.placas,unidades.placas ) placas, IFNULL( unidades.descripcion, '-----' ) descUnidad, 
	ROUND( IF( cli.tipodepago REGEXP 'Credito|Prepago' AND rm.kilometraje > IFNULL( rma.kilometraje, 0 ), rm.kilometraje - IFNULL( rma.kilometraje, 0 ), 0 ) / rm.volumen, 2 ) rendimiento,
	IFNULL( ven.alias, '' ) alias
FROM (
	SELECT 
		rm.*, 
		IFNULL( ( SELECT MAX( ba.id_consumo ) FROM bitacora_autorizaciones ba WHERE ba.codigo = rm.codigo AND ba.id_consumo < rm.id ), 0 ) idant 
        FROM rm FORCE INDEX( corte_dis_mang_idx )
        JOIN man_pro mp ON mp.posicion = rm.posicion AND mp.manguera = rm.manguera
	JOIN ct ON ct.id = rm.corte 
	WHERE TRUE $cWhere
) rm 
...
LEFT JOIN rm rma ON rm.idant = rma.id
LEFT JOIN ven ON ven.id = rm.vendedor
LEFT JOIN unidades ON rm.codigo = unidades.codigo 
WHERE man.activo = 'Si' $cWhereTC AND rm.cliente > 0 AND rm.tipo_venta in ('D','N','C') " . (is_numeric($IslaPosicion) ? " AND man.isla_pos = $IslaPosicion" : "") . "
UNION ALL
SELECT 
	'1' tipo, 
	cli.tipodepago, vt.cliente, cli.nombre,
	man.isla_pos, vt.corte, vt.id ticket, vt.cantidad volumen, vt.total importe, vt.total pagoreal, 0 ieps, vt.iva,
	inv.descripcion descripcion, inv.descripcion producto,
	vt.fecha, vt.uuid, 
	IF( vt.codigo REGEXP '([0-9]{10,13}[|]?)+', 'Vales', vt.codigo ) codigo, IFNULL( unidades.impreso, '-----' ) impreso, IFNULL( unidades.numeco, '-----' ) numeco, 
        IFNULL( unidades.placas, '-----' ) COLLATE utf8_general_ci placas, IFNULL( unidades.descripcion, '-----' ) descUnidad, 
    0 kilometraje, 0 rendimiento, '-----' alias  
FROM vtaditivos vt
JOIN inv ON inv.id = vt.clave
JOIN cli ON cli.id = vt.cliente
JOIN ct ON ct.id = vt.corte
JOIN man ON man.posicion = vt.posicion 
LEFT JOIN unidades ON vt.codigo = unidades.codigo 
WHERE vt.tm = 'C' AND vt.cliente > 0 AND man.activo = 'Si' $cWhereA
UNION ALL
SELECT 
	'3' tipo, 
	cli.tipodepago, cli.id cliente, cli.nombre,
	0 isla_pos, cttarjetas.id corte, cttarjetas.idnvo ticket, 0 volumen, cttarjetas.importe, cttarjetas.importe pagoreal, 0 ieps, 0.16 iva,
	'' descripcion,  '' producto, 
	cttarjetas.fecha, '-----' uuid, 
	'' codigo, '' impreso, 
	'' numeco, IFNULL( cxc.placas, '' ), 'T' descUnidad, 
    0 kilometraje, 0 rendimiento, '-----' alias  
FROM cttarjetas
JOIN cli ON cttarjetas.banco = cli.id
JOIN ct ON cttarjetas.id = ct.id
LEFT JOIN cxc ON cttarjetas.id = cxc.referencia AND cxc.tm = 'C' AND cxc.producto = '-'
WHERE cli.tipodepago = 'Tarjeta'  $cWhereT
ORDER BY tipo, cliente, impreso, fecha, ticket";

if ($Clave != "*" && $Clave != "Aditivos") {
...
$selectConsumosTotalesByProducto = "
        ...
        FROM ct
            JOIN rm USE INDEX(corte_dis_mang_idx) ON rm.corte = ct.id
            JOIN cli ON cli.id = rm.cliente
            JOIN man ON rm.posicion = man.id 
            JOIN com ON rm.producto = com.clavei 
        WHERE  rm.tipo_venta in ('D','N')
        AND rm.cliente > 0 $cWhere $cWhereBP
...
$selectFacturasPendientes = "
            SELECT cxc.*,p.fecha fechap,p.fecha_deposito,p.serie seriep FROM (
                ...
                ROUND(fc.total,2) total,
                 ROUND(fc.descuento,2) descuento,
                ROUND((fc.total - fc.descuento) - IFNULL(A.abono,0),2) saldo,
