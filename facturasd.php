...
$cSQL = "SELECT CONCAT(fc.serie, ' ', fc.folio) folio, fc.status, fc.uuid, fc.fecha, fc.cliente,fc.observaciones,fc.periodo,fc.meses, 
        ROUND(fc.cantidad, 2) cantidad,ROUND(fc.importe + fc.iva + fc.ieps, 2 ) subtotal,fc.descuento, fc.ano,fc.relacioncfdi,
...
$He = $HeA->fetch_array();
if ($request->getAttribute("Op") === "ModificaPago") {
    $UpdPago = "UPDATE pagos SET importe = '" . $He["total"] . "' WHERE  id = " . $He["relacioncfdi"];
    utils\IConnection::execSql($UpdPago);
    $UpdCxc = "UPDATE cxc SET importe = '" . $He["total"] . "' WHERE tm='H' AND recibo = " . $He["relacioncfdi"];
    utils\IConnection::execSql($UpdCxc);
    $BitacoraDAO = new BitacoraDAO();
    $BitacoraDAO->saveLogSn($usuarioSesion->getNombre(), "ADM", "Modifica el pago con base a la factura");
    header("location: facturasd.php");
}
...
<html lang="es" xml:lang="es">
    ...
    <body>
        ...
        <div id="DatosEncabezado" style="border: 1px solid #808B96;">
            <?php
            $Vc = "SELECT valor FROM variables_corporativo WHERE llave = 'LiberarTickets';";
            $Vc_ = utils\IConnection::execSql($Vc);
            if ($fcVO->getStatus() == 2 && ($usuarioSesion->getTeam() === "Administrador" || $usuarioSesion->getTeam() === "Supervisor") && $Vc_["valor"] == 1) {
                ?>
            ...
            <table aria-hidden="true">
                ...
                <?php
                if (number_format(round($He["importe"] + $He["ieps"] + $He["iva"], 2), 2) <> number_format($He["pagoImporte"], 2) && $He["uuid"] === "-----" && $He["pagoImporte"] > 0) {
                    ?>
                    <tr>
                        <td colspan="7" style="text-align: right;"><input type="button" name="Boton" id="BotonGeneral" value="Modificar pago al importe de factura"></td>
                    </tr>
                    <?php
                }
                ?>
                                ...
                                <div class="container no-margin">
                                    <div class="row no-padding">
                                        <div class="col-12 background container no-margin">
                                            ...
                                            <?php
                                            if ($primerIf) :
                                                $sql = "SELECT valor FROM variables_corporativo WHERE llave = 'validaTicketdeImporte';";
                                                $rsVl = utils\IConnection::execSql($sql);
                                                $RsV = $rsVl["valor"] == 1 ? "button" : "submit";
                                                ?>
                                                <div class="row no-padding">
                                                    ...
                                                    <div class="col-5 "><input type="<?= $RsV ?>" name="Boton" value="Agregar ticket" id="PorTicket"></div>
    ...
    ?>
                $(document).on('keydown', function (e) {
                    if (e.key === 'Enter' || e.keyCode === 13) {
                        e.preventDefault(); // Bloquea el Enter globalmente
                    }
                });
    <?php
}
    ...
    ?>
                    $("#PorTicket").click(function () {
                        jQuery.ajax({
                            ...
                            dataType: "json",
                            cache: false,
                            data: {"Op": "ConsultaTicket", "idRm": $("#Ticket").val(), "Tipo": $('input[type="radio"][name="Tipo"]:checked').val()},
                            success: function (data) {
                                console.log(data);
                                if (data.result == 1) {
                                    ...
                                    Swal.fire({
                                        title: 'Este ticket ya presenta un proceso en curso o finalizado. Verifique su estado antes de continuar.',
                                        icon: 'error',
                                        iconColor: 'red',
                                    });
                                }
...
?>
                ...
                $("#BotonGeneral").click(function () {
                    Swal.fire({
                        title: "¿Está seguro de que desea transferir el importe de la factura al monto del  pago y reflejarlo también en el estado de cuenta?",
                        icon: "info",
                        background: "#E9E9E9",
                        showConfirmButton: true,
                        showCancelButton: true,
                        ...
                        if (result.isConfirmed) {
                            window.location.href = "facturasd.php?Op=ModificaPago";
                        }
                    });
                });
