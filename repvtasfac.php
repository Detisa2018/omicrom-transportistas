    ...
    if ($request->hasAttribute("Fecha")) {
        ...
        $cSql_VtAditivos = "SELECT 
                ...
                LEFT JOIN (
                    ...
                    GROUP BY cli.tipodepago) F 
                ON F.tipodepago = V.tipodepago
                UNION ALL SELECT  'x' cliente,'.' alias,'tp'tip,Count(1) cnt, 0 v, SUM(total), 0 m  
                FROM fc WHERE id IN (SELECT id FROM fcd WHERE id in 
                (SELECT id FROM omicrom.fc WHERE DATE(fecha) between DATE('$FechaI') AND date('$FechaF') AND status = 1) AND ticket = 0);";
                        ...
                        if (($Vta2 = $mysqli->query($cSql))) {
                            while ($rg = $Vta2->fetch_array()) {
                                ...
                                if ($rg["tipodepago"] == "CONTADO") {
                                    ...
                                    echo "<td class=\"numero\"><a class='textosCualli' href=javascript:window.winieps('penporfacturar.php?tpo=1&op=$op&FechaI=$FechaI&FechaF=$FechaF&Fecha=$Fecha')>" . number_format($rg["Porfacturar"], 2) . "</a></td>";
                                    ...
                                    echo "<td class=\"numero\"><a class='textosCualli' href=javascript:window.winieps('penporfacturar.php?tpo=4&op=$op&FechaI=$FechaI&FechaF=$FechaF&Fecha=$Fecha')>" . number_format($rg["Porfacturar"], 2) . "</a></td>";
