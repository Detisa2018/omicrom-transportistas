...
if ($request->getAttribute("TipoS") === "Cnt") {
    ...
    cli.nombre,
    fc.uuid,fcd.ticket,
    ROUND(fcd.cantidad,3) cantidad,
    ROUND((fcd.cantidad * fcd.precio),2) importe,
    ROUND((fcd.cantidad * fcd.precio) * (fcd.iva),2) iva,
...
FROM
    fcd
        JOIN
    (SELECT 
        fcd.ticket, COUNT(*) AS movimientos
    FROM
        rm
    LEFT JOIN fcd ON fcd.ticket = rm.id
    LEFT JOIN fc ON fcd.id = fc.id
    WHERE
        MONTH(rm.fecha_venta) = $mes
            AND YEAR(rm.fecha_venta) = $anio
            AND rm.uuid <> '-----'
            AND rm.tipo_venta = 'D'
            AND fc.status = 1
            AND fcd.producto < 5
    GROUP BY fcd.ticket
    HAVING COUNT(*) > 1) AS subquery ON fcd.ticket = subquery.ticket
    inner join fc on fcd.id = fc.id
    inner join cli on cli.id = fc.cliente;   
    ";
}
    ...
    <body>
        <div id="container">
            ...
            <div id="Reportes">
                        ...
                        <tbody>
                            ...
                            foreach ($registros as $rg) {
                                ...
                                <tr>
                                    ...
                                    <td><?= $rg["uuid"] ?></td>
                                    <td><?= $rg["ticket"] ?></td>
                                    <td style="text-align: right;"><?= number_format($rg["cantidad"], 3) ?></td>
                ...
                <?php
                $TicketsDuplicadosAditivos = "SELECT fcd.id idFc ,sUM(fcd.importe) importe ,fc.uuid uuidFc ,
                    v.id idAd ,SUM(v.total) Total,v.uuid uuidA FROM fc LEFT JOIN fcd ON fc.id = fcd.id  
                                LEFT JOIN vtaditivos v ON v.id = fcd.ticket WHERE
                                YEAR(fc.fecha) = '$anio' AND MONTH(fc.fecha) = '$mes'  AND status = 1 AND fcd.producto > 5 AND fcd.ticket > 0
                                AND v.uuid='-----' group by fcd.ticket order by v.uuid;";
                $Ta = utils\IConnection::getRowsFromQuery($TicketsDuplicadosAditivos);
                ?>
                <table aria-hidden="true">
                    <thead>
                        <tr class="titulo">
                            <td colspan="12">Discrepancias entre factura y su detalle Aditivos</td>
                        </tr>
                        <tr>
                            <td>Factura</td>
                            <td>Importe</td>
