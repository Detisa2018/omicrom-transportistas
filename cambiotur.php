...
$paginador = new Paginador($Id,
        "ct.status,ct.usr,ct.statusctv,enviado_grupo ",
        "",
        "",
        "$Add",
...
$vGs = ($RsBGes["valor"] === "" || $RsBGes["valor"] == null || $RsBGes["valor"] == 0) ? false : true;
...
$SqlAuth = "SELECT groupwork FROM authuser WHERE id = " . $usuarioSesion->getId();
$Group = utils\IConnection::execSql($SqlAuth);
?>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $(".openThis").click(function () {
                    if (("<?= $usuarioSesion->getTeam() ?>" === "Administrador" && "<?= $Group["groupwork"] ?>" === "1") && "<?= utils\IConnection::execSql("SELECT valor FROM variables_corporativo WHERE llave='habilita_cortes'")["valor"] ?>" === "1") {
                ...
                $(".fa-paper-plane").click(function () {
                    var idCorte = this.dataset.idinfo;
                    Swal.fire({
                        icon: "question",
                        iconColor: "#5499C7",
                        ...
                        if (result.isConfirmed) {
                            jQuery.ajax({
                                ...
                                dataType: "json",
                                cache: false,
                                data: {"Origen": "GeneraProcesoGrupoG", "Corte": idCorte},
                                success: function () {
                                    Swal.fire({
                                        title: "Archivos enviados con exito!"
                                    }).then(() => {
                                        var Msj = "Datos enviados con exito!";
                                        $(location).attr('href', "cambiotur.php?Msj=" + Msj);
                                    });
                                }
                            });
                        }
            ...
            <table class="paginador" aria-hidden="true">
                ...
                while ($paginador->next()) {
                    ...
                    if ($row["status"] === StatusCorte::ABIERTO) {
                        ...
                        echo "<td>";
                        if ($row["statusctv"] === StatusCorte::CERRADO) {
                            $VariablesC = "SELECT valor FROM variables_corporativo WHERE llave = 'PermisoCorteAbierto'";
                            $vc = utils\IConnection::execSql($VariablesC);
                            $AddClass = "";
                            if ($vc["valor"] == $usuarioSesion->getId()) {
                                $AddClass = "openThis";
                            }
                            ...
                            if ($rsenv["id"] > 0) {
                                echo "<div class='IconAzulFull $AddClass' title='Corte cuadrado y enviado a banco' data-corte='" . $row["id"] . "'></div>";
                            } else {
                                echo "<div class='IconVerde $AddClass' title='Corte cuadrado' data-corte='" . $row["id"] . "'></div>";
//                                echo "<td><div><a><i class='fa-solid fa-envelope'></i></a></div>";
                        ...
                        if ($vGs) {
                            echo "<td>";
                            if ($vGs && $row["statusctv"] === StatusCorte::CERRADO) {
                                $colorIcon = $row["enviado_grupo"] == 1 ? "#28B463" : "#E74C3C";
                                $datetime = new DateTime($row["fecha"]);
                                $fecha = $datetime->format('Y-m-d');
                                if ($fecha < date("Y-m-d")) {
                                    echo "<i class=\"fa-solid fa-paper-plane\" data-idinfo='" . $row["id"] . "' style='color: $colorIcon'></i>";
                                }
                            }
                            echo "</td>";
                        }
