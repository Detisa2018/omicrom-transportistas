...
class CatalogosSelectores {
    ...
    static function generateClaveCP($comboID, $StyleAdd = "") {
        $mysqli = iconnect();
        $array = array();
        $qry = $mysqli->query("SELECT * FROM cfdi33_c_conceptos WHERE status = 1;");
    ...
    static function generateClaveCPDis($comboID, $StyleAdd = "") {
        $mysqli = iconnect();
        $array = array();
        $qry = $mysqli->query("SELECT * FROM cfdi33_c_conceptos WHERE status = 1;");
        $html = "<select disabled class = \"form-control\" style=\"font-size: 11px; font-family: monospace;$StyleAdd\" id=\"" . $comboID . "\" name=\"" . $comboID . "\"><option value=\"000000\">SELECCIONE CLAVE CP</option>";
        while (($rs = $qry->fetch_array())) {
    ...
    static function getMotivos_Cancelacion($CliRfc = "") {
        $mysqli = iconnect();
        $array = array();
        $AddConsulta = $CliRfc === "XAXX010101000" ? " AND clave = '04' " : " AND clave = '02' ";
        $Sql = "SELECT clave, descripcion FROM cp_motivo_cancelacion WHERE activo = 1 $AddConsulta";
        $qry = $mysqli->query($Sql);
        while (($rs = $qry->fetch_array())) {
            $array[$rs["clave"]] = $rs["clave"] . " | " . $rs["descripcion"];
        }
...
class ComboboxPermisosTerminales {

    static function generate($comboID) {
        $mysqli = iconnect();
        $Sql = "SELECT id clave, CONCAT(descripcion, ' - ', permiso) descripcion FROM permisos_cre WHERE catalogo = 'TERMINALES_ALMACENAMIENTO' AND id > 0;";
        $qry = $mysqli->query($Sql);

        $html = "<select style = 'width: $width' class = 'texto_tablas' name = '$comboID' id = '$comboID'><option value = ''>SELECCIONE TERMINAL</option>";
        while (($rs = $qry->fetch_array())) {
            $html .= "<option value = '" . $rs["clave"] . "'>" . $rs["descripcion"] . "</option>";
        }
...
class ComboboCatalogos {

    static function generate($comboID) {
        $mysqli = iconnect();
        $Sql = "SELECT clave,descripcion FROM omicrom.cfdi33_c_pais WHERE status = 1;";
        $qry = $mysqli->query($Sql);

        $html = "<select style = 'width: $width' class = 'texto_tablas' name = '$comboID' id = '$comboID'><option value = ''>SELECCIONE ORIGEN</option>";
        while (($rs = $qry->fetch_array())) {
            $html .= "<option value = '" . $rs["clave"] . "'>" . $rs["descripcion"] . "</option>";
        }
