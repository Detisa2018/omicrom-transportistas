...
require './services/FacturasService.php';
...
$sql = "SELECT fc.fecha, fc.cliente, fc.status, COUNT( fcd.id ) items, fc.id,
        ...
        cia.rfc emisor, cli.nombre, cli.tipodepago, fc.uuid, IFNULL( fc.usr, 'Unknown' ) usr, fc.serie, fc.folio, fc.relacioncfdi
  	FROM fc 
        JOIN fcd ON fc.id = fcd.id
        JOIN cia ON TRUE
        LEFT JOIN cli ON cli.id = fc.cliente 
        LEFT JOIN facturas ON facturas.id_fc_fk = fc.id AND facturas.uuid = fc.uuid 
        WHERE fc.id = " . $busca . " GROUP BY fcd.id";
...
$statusCFDI = array();
if ($Cpo["status"] != StatusFactura::ABIERTO) {
    $statusCFDI = ConsultaCFDIClient::CallAPI($Cpo['rfcEmisor'], $Cpo['rfcReceptor'], $Cpo['cfditotal'], $Cpo['uuid']);
}

$sqlRelacion = "SELECT 'fc' tabla, id, uuid FROM fc WHERE relacioncfdi = " . $busca .
        " UNION ALL
                SELECT 'nc' tabla, id, uuid FROM nc WHERE relacioncfdi = " . $busca .
        " UNION ALL
                SELECT 'pg' tabla, id, uuid FROM pagos WHERE relacioncfdi = " . $busca;
error_log($sqlRelacion);
$relaciones = $mysqli->query($sqlRelacion)->fetch_array();
if ($relaciones) {
    $relacionados = "";
    if ($relaciones['tabla'] == 'fc') {
        $relacionados = "<br/><a class=\"textosCualli\" href=\"canfactura.php?busca=" . $relaciones['id'] . "\">Cancelar Factura " . $relaciones['uuid'] . "</a><br />";
    } else if ($relaciones['tabla'] == 'nc') {
        $relacionados = "<br/><a class=\"textosCualli\" href=\"cannotascre.php?busca=" . $relaciones['id'] . "\">Cancelar Nota de Crédito " . $relaciones['uuid'] . "</a><br />";
    } else if ($relaciones['tabla'] == 'pg') {
        $relacionados = "<br/><a class=\"textosCualli\" href=\"cantimbrepago.php?busca=" . $relaciones['id'] . "\">Cancelar Pago " . $relaciones['uuid'] . "</a><br />";
    }
}
$Mensaje = "";
$Cancelar = false;
if ($Cpo['status'] == StatusFactura::CANCELADO && (empty($Cpo['uuid']) || $Cpo['uuid'] === FcDAO::SIN_TIMBRAR)) {
...
} else if ($Cpo['items'] > 1 && ($Cpo['rfcReceptor'] === FcDAO::RFC_GENERIC && $Cpo['tipodepago'] === "Contado")) {
    $Cancelar = true;
    ...
    if ($Cpo['tipodepago'] === TiposCliente::PREPAGO) {
        ...
        if ($pagoVO->getId() > 0) {
            if ($pagoVO->getStatus_pago() == StatusPagoPrepago::CON_NOTA_CREDITO) {
                $selectNc = "SELECT * FROM nc WHERE relacioncfdi = '$busca' AND status = '" . StatusNotaCredito::CERRADO . "'";
    ...
    } elseif ($Cpo['tipodepago'] === TiposCliente::CREDITO) {
            ...
            <tr>
                ...
                <td style="vertical-align: top;">
                    <form name="form1" id="form1" method="post" action="">
                        ...
                        if (!empty($Cpo['uuid']) && $Cpo['uuid'] !== FcDAO::SIN_TIMBRAR) {
                        ...
                        $SelectVariables = "SELECT valor FROM variables_corporativo WHERE llave ='RolesCancelan'";
                        $VlP = utils\IConnection::execSql($SelectVariables);
                        $ValoresExp = explode(",", $VlP["valor"]);
                        if ($statusCFDI['Estado'] !== "No Encontrado" && ($usuarioSesion->getTeam() === typeTeam::ADMINISTRADOR || $usuarioSesion->getTeam() === typeTeam::SUPERVISOR || in_array($usuarioSesion->getTeam(), $ValoresExp))) {
                            if ($Cancelar) {
                                $CancelacionDAO = new CancelacionDAO();
                                $CancelacionVO = new CancelacionVO();
                                $CancelacionVO = $CancelacionDAO->retrieve($busca, "fc");
                                if ($CancelacionVO->getDescripcion_evento() <> null) {
                                    ...
                                    <tr height='21' class='texto_tablas'>
                                        ...
                                        <td>
                                            &nbsp;<select name="TipoCancelacion" id="TipoCancelacion" class="texto_tablas">
                                                <?php
                                                $arrayDatos = CatalogosSelectores::getMotivos_Cancelacion($Cpo['rfcReceptor']);
                                    ...
                                    <tr height='21' class='texto_tablas'>
                                        ...
                                        <td>
                                            <textarea name="MotivoCancelacion" id="MotivoCancelacion" cols="40" rows="5" required="required"></textarea>
                                        </td>
                                    </tr>
                                    <tr>
                        ...
                        <div class="texto_tablas" style="text-align: left; width: 95%; padding: 3px;color: red">
                            <?php if (!empty($Cpo['uuid']) && $Cpo['uuid'] !== FcDAO::SIN_TIMBRAR && contains($statusCFDI['Estado'], "No Encontrado")) { ?>
