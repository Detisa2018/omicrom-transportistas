                        ...
                        <div id="TablaDatosReporte"> <!-- Abre div estilos-->
                            <div style="padding-top: 10px;">
                                <table aria-hidden="true" style="max-width: 200px !important;"> <!-- Abre tabla 1-->
                                    <thead>
                                        <tr  class="<?= $ignore ?>" style="font-size: 11px;">
                                            <td>#</td>
                                            <td>Entrada</td>
                                            <td>No.Fact</td>
                                            <td>Terminal</td>
                                    ...
                                    $sql = "SELECT me.id,me.foliofac,me.volumenfac,prv.nombre,me.uuid,me.tipocomprobante,me.tipo FROM me inner join prv on me.proveedor = prv.id WHERE me.carga = '" . $rg["carga"] . "'";
                                    ...
                                    <tr style="font-size: 11px;">              
                                        ...
                                        <td class="numero"><?= number_format($rg["diferencia"] + $rg["vd"], 3) ?></td>
                                    ...
                                    while (($dt = $query->fetch_array())) {
                                        $HtmlAdd = $dt["tipo"] === "Consignacion" ? 'style="background-color: #ABEBC6 !important;"' : "";
                                        ?>
                                        <tr class="subt">
                                            <td <?= $HtmlAdd ?>><?= $dt["id"] ?></td>
                                            <td><?= $dt["tipocomprobante"] === "E" ? "N. C." : "I." ?> - <?= $dt["tipo"] ?></td>
                                            <td><?= $dt["foliofac"] ?></td>
                            ...
                            <table aria-hidden="true">
                                ...
                                <tbody>
                                    ...
                                    foreach ($registrosT as $rg) {
                                        echo $rg["concatIdCarga"] . "<br>";
                                        $Sql2 = "SELECT SUM(ca.aumento) incremento, SUM(ca.tcaumento) neto FROM cargas ca WHERE date(ca.fecha_insercion) 
                                            BETWEEN DATE('$FechaI') AND DATE('$FechaF') 
                                            AND ca.producto='" . $rg["descripcion"] . "' AND ca.entrada > 0 AND ca.tipo = 0  group by producto";
                                        $rs2 = utils\IConnection::execSql($Sql2);
                                        ?>
                                        <tr class="tableexport-ignore">                 
                                            ...
                                            <td class="numero"><?= $rg["tipocarga"] === "Consignacion" ? 0.00 : number_format($rs2["incremento"], 0) ?></td>
                                            <td class="numero"><?= number_format($rs2["neto"], 0) ?></td>
