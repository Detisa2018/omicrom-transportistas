        ...
        <div class="sheet padding-10mm">
            ...
            <div id="exportContent">
                ...
                <table style="width: 100%;" aria-hidden="true">
                    ...
                    $Correo = "SELECT valor from variables_corporativo where llave like '%CorreoArchivosCia%';";
                    $CorreoD = utils\IConnection::execSql($Correo);
                    ?>
                    ...
                    <tr><td><strong>Correo electrónico: </strong><?= $CorreoD["valor"] ?> </td></tr>
                    ...
                    <tr><td><strong>Teléfono de proveedor:</strong> 595 92 504 01 y 55 6255 3855</td></tr>
                    ...
                    <?PHP
                    $ServidorC = "SELECT * FROM omicrom.pos_catalog WHERE dispositivo = 'C' AND status = 'A';";
                    $ServC = utils\IConnection::execSql($ServidorC);
                    ?>
                    <tr><td><strong>Modelo / Nombre: <?= $ServC["model"] ?></strong></td></tr>
                    <tr><td><strong>Número de Serie / Versión: <?= $ServC["serial"] ?></strong></td></tr>
                    <tr><td><strong>Funciones que realiza:</strong> Seguridad para la red de control Volumétricos. </td></tr>
                    <tr><td><strong>Ubicación física dentro de la E.S.:</strong> Oficina Operativa.</td></tr> 
                    <tr><td style="height: 15px;"></td></tr>
                    <?PHP
                    $ServidorI = "SELECT * FROM omicrom.pos_catalog WHERE dispositivo = 'I' AND status = 'A';";
                    ...
                    <tr>
                        <td>
                            <table class="StyleTable" summary="Datos generales de la instalación">
                                ...
                                <?php
$Dispensarios = "SELECT * FROM ( 
    SELECT 
        lv.valor_lista_valor, 
        man.isla_pos isla, 
        man.posicion, 
        dispS.serial,
	dispS.model
    FROM 
        man
    LEFT JOIN 
        man_pro ON man_pro.posicion = man.posicion
    LEFT JOIN 
        listas_valor lv ON man.marca = lv.llave_lista_valor
    LEFT JOIN 
        (SELECT @i := @i + 1 AS idt, p.* 
        FROM pos_catalog p 
        CROSS JOIN (SELECT @i := 0) p1 
        WHERE p.status = 'A' AND p.dispositivo = 'D') dispS 
    ON 
        man.isla_pos = dispS.idt
    WHERE 
        man.activo = 'Si'
        AND dispS.serial IS NOT NULL 
        AND dispS.model IS NOT NULL
    GROUP BY 
        man.isla_pos
    ORDER BY 
        man.isla_pos
...
LEFT JOIN ( 
    SELECT @i := @i + 1 as idt, p.* 
    FROM pos_catalog p 
    CROSS JOIN (SELECT @i := 0) p 
    WHERE p.status = 'A' AND p.dispositivo = 'D' 
) dispS ON dispP.isla = dispS.idt";

$rsDsp = utils\IConnection::getRowsFromQuery($Dispensarios);
$i = 1;
foreach ($rsDsp as $Dsp) {
    $SubSql = "SELECT com.descripcion, clave, count(1) num 
               FROM omicrom.man_pro mp 
               LEFT JOIN com ON mp.producto = com.clavei 
               WHERE mp.dispensario = " . $Dsp["isla"] . " 
               AND mp.activo = 'Si' 
                        ...
                        <td>
                            <table class="StyleTable"  summary="Descripcion de cada uno de los tanques">
                                <tr>
                                    ...
                                    <th>Vol. Operativo (litros)</th>
                                    <th>Tipo de Combustible</th>
                                    <th style="width: 35%;">Interconectado con los tanques para transvase (indique cuales)</th>
                                </tr>
                                <?php
                                $Dispensarios = "SELECT tanque,capacidad_total,producto,volumen_operativo FROM omicrom.tanques  where estado = 1;";
                                $rsDsp = utils\IConnection::getRowsFromQuery($Dispensarios);
                                $i = 1;
                                foreach ($rsDsp as $Dsp) {
                                    ?>
