...
if ($request->hasAttribute("Boton")) {
    ...
    $UnidadesDAO = new TarjetaDAO();
    $UnidadesVO = new TarjetaVO();
    $UnidadesVO = $UnidadesDAO->retrieve($busca);
    $txt = (string) $UnidadesVO;
    ...
    if ($mysqli->query($updateUnidades)) {
        ...
        $txt .= " |||| ";
        $txt .= (string) $UnidadesVO;
        $BitacoraDAO->saveLog($usuarioSesion->getNombre(), "cli", $txt);
