...
switch ($Op) {
    case "Visor":
        ...
        $Html .= "</table>";
        $TotalFactura = "SELECT total FROM fc WHERE id = " . $request->getAttribute("IdFactura");
        $TotalFactura = utils\IConnection::execSql($TotalFactura);
        $ImpPago = "SELECT importe,total FROM fc WHERE id = '" . $request->getAttribute("CliHd") . "'";
        ...
        foreach ($RowSql as $row) {
            $totalCalculado = $TotalFactura["total"] - $TT <= $row["res"] ? $TotalFactura["total"] - $TT : $row["res"];
            $Html .= '<tr><td style="text-align:left;">Folio : ' . $row["id"] . ' Serie : ' . $row["serie"] . '</td><td style="text-align:right;">' . number_format($row["total"], 2) . '</td>'
                    ...
                    . '<td style="text-align:center;" class="Facturado">';
            if ($TotalFactura["total"] - $TT > 0) {
                $Html .= '<i class="fa fa-arrow-circle-up GuardaFactura" aria-hidden="true" data-id="' . $row["id"] . '" data-uuid="' . $row["uuidpc"] . '" data-total="' . $totalCalculado . '"></i>';
            }
            $Html .= '</td>'
                    . '</tr>';
        }
