...
include_once ("data/Ct_cerradoDAO.php");
include_once ("data/AuthSemestralResultadosDAO.php");
...
}else if ($request->getAttribute("Origen") === "ComprasDetalle") {
    $numpiezas = $request->getAttribute("numpiezasR");
    $idnvo = $request->getAttribute("idnvoCompra");

    // Asegúrate de sanitizar o parametrizar esto si usas raw SQL
    $Update = "UPDATE etd 
           SET costo = ROUND((costo * cantidad) / $numpiezas ,2),
               cantidad = $numpiezas 
           WHERE idnvo = $idnvo";
...
LEFT JOIN ct ON id_corte = ct.id WHERE id_ee = " . $request->getAttribute("IdBusca") . " ORDER BY id_corte DESC";
    ...
    $Html .= "</table></td></tr>";
    $display["Html"] = $Html;
} else if ($request->getAttribute("Op") === "ObtenUnidades") {
    $UnidadesCliente = "SELECT id,CONCAT(descripcion,' - ' , codigo,' - ',numeco,' - ',IF(periodo='B',CONCAT('$',importe),'')) descripcion "
            . "FROM unidades WHERE cliente = " . $request->getAttribute("IdCli") . " AND estado = 'a';";
    $Uc = utils\IConnection::getRowsFromQuery($UnidadesCliente);
    foreach ($Uc as $rs) {
        $display[$rs["id"]] = $rs["descripcion"];
    }
} else if ($request->getAttribute("Op") === "ModificaVentaAditivoIsla") {
    $BitacoraDAO = new BitacoraDAO();
    $BitacoraDAO->saveLogSn($request->getAttribute("Usr"), "ADM", "Transferencia de venta de isla  " . $request->getAttribute("IslaOrigen") . " a  " . $request->getAttribute("IslaDestino") . ", producto : " . $request->getAttribute("Producto"));
    $Sql = "SELECT idnvo FROM invd WHERE isla_pos = " . $request->getAttribute("IslaDestino") . " AND id = " . $request->getAttribute("Producto");
    $idModifica = utils\IConnection::execSql($Sql);
    if ($idModifica["idnvo"] > 0) {
        $Sql2 = "UPDATE invd  SET existencia = existencia + " . $request->getAttribute("Cantidad") . " WHERE idnvo = " . $idModifica["idnvo"];
    } else {
        $Sql2 = "INSERT INTO invd (id,isla_pos,minimo,maximo,existencia,modificacion) "
                . "VALUES  ('" . $request->getAttribute("Producto") . "','" . $request->getAttribute("IslaDestino") . "',0,0," . $request->getAttribute("Cantidad") . ",'" . date("Y-m-d H:i:s") . "');";
    }
    $Update = "UPDATE invd  SET existencia = existencia - " . $request->getAttribute("Cantidad") . " WHERE isla_pos = " . $request->getAttribute("IslaOrigen") . " AND id = " . $request->getAttribute("Producto");

    $Despachador = "SELECT despachador FROM man where isla_pos=" . $request->getAttribute("IslaDestino");
    $idDesp = utils\IConnection::execSql($Despachador);
    $Update2 = "UPDATE vtaditivos SET posicion =  " . $request->getAttribute("IslaDestino") . ",vendedor = " . $idDesp["despachador"] . " WHERE id =" . $request->getAttribute("IdVt");
    utils\IConnection::execSql($Update);
    utils\IConnection::execSql($Update2);
    utils\IConnection::execSql($Sql2);
} else if ($request->getAttribute("Op") === "CrearCodigoCliente") {
    $IdCli = $request->getAttribute("Cliente");
    $Cia = "SELECT LPAD((idfae), 4, '0') num FROM cia";
    $Ci = utils\IConnection::execSql($Cia);
    $Unidad = "SELECT COUNT(1) + 1 num FROM unidades WHERE cliente = $IdCli;";
    $Cu = utils\IConnection::execSql($Unidad);
    $Cod = strlen($Cu["num"]) > 0 ? str_pad($Cu["num"], 7, '0', STR_PAD_LEFT) : str_pad(1, 7, '0', STR_PAD_LEFT);

    $Codigo = $Ci["num"] . str_pad($IdCli, 6, '0', STR_PAD_LEFT) . $Cod;

    $display["Codigo"] = $Codigo;
} else if ($request->getAttribute("Op") === "CreaciónAsignacionCodigo") {
    $Insert = "INSERT INTO unidades (descripcion,cliente,codigo,impreso,combustible) "
            . "VALUES ('Codigo creado', " . $request->getAttribute("Cliente") . "  ,'" . $request->getAttribute("CodigoNvo") . "','" . $request->getAttribute("CodigoNvo") . "','123');";
    utils\IConnection::execSql($Insert);
    BitacoraDAO::getInstance()->saveLogSn($request->getAttribute("Usr"), "ADM", "Se crea codigo cliente  " . $request->getAttribute("Cliente") . " : " . $request->getAttribute("CodigoNvo"));
} else if ($request->getAttribute("Op") === "Log_Cortes") {
    $Ct_cerradoVO = new Ct_cerradoVO();
    $Ct_cerradoDAO = new Ct_cerradoDAO();

    $Ct_cerradoVO->setId_ct($request->getAttribute("id_ct"));
    $Ct_cerradoVO->setId_usr($request->getAttribute("id_usr"));
    $Ct_cerradoVO->setFecha(date("Y-m-d H:i:s"));
    $Ct_cerradoVO->setTotal($request->getAttribute("total"));
    $Ct_cerradoVO->setCredito($request->getAttribute("clientes"));
    $Ct_cerradoVO->setBancos($request->getAttribute("tarjetas"));
    $Ct_cerradoVO->setConsignaciones($request->getAttribute("consignaciones"));
    $Ct_cerradoVO->setMonederos($request->getAttribute("monederos"));
    $Ct_cerradoVO->setAceites($request->getAttribute("aceites"));
    $Ct_cerradoVO->setDolares($request->getAttribute("dolares"));
    $Ct_cerradoVO->setGastos($request->getAttribute("gastos"));
    $Ct_cerradoVO->setEfectivo($request->getAttribute("efectivo"));
    $Ct_cerradoVO->setDepositos($request->getAttribute("despositos"));
    $Ct_cerradoDAO->create($Ct_cerradoVO);
} else if ($request->getAttribute("Op") === "ResultadoReporte") {
    $Update = "UPDATE ordenes_atendidas SET calificacion = " . $request->getAttribute("Calificacion") . ", observaciones = '" . $request->getAttribute("Observacion") . "', "
            . "fecha_respuesta = '" . date("Y-m-d H:i:s") . "', id_user_respuesta = " . $request->getAttribute("Usr") . " WHERE id = " . $request->getAttribute("id_Orden");
    utils\IConnection::execSql($Update);
} else if ($request->getAttribute("Op") === "ObtenUnidades2") {
    $UnidadesCliente = "SELECT codigo id,CONCAT(descripcion,' - ' , codigo,' - ',IF(periodo='B',CONCAT('$',importe),'')) descripcion "
            . "FROM unidades WHERE cliente = " . $request->getAttribute("IdCli") . " AND estado = 'a';";
    $Uc = utils\IConnection::getRowsFromQuery($UnidadesCliente);
    foreach ($Uc as $rs) {
        $display[$rs["id"]] = $rs["descripcion"];
    }
} else if ($request->getAttribute("Op") === "ConsultaTicket") {
    if ($request->getAttribute("Tipo") === "C") {
        $SqlTicket = "SELECT IF((pesos - importe) < 1, true, false) value FROM rm WHERE id = " . $request->getAttribute("idRm");
        $res = utils\IConnection::execSql($SqlTicket);
    } else if ($request->getAttribute("Tipo") === "A") {
        $res["value"] = true;
    }
    $display["result"] = $res["value"];
} else if ($request->getAttribute("Op") === "RegistraBitacoraSemestral") {
    $AuthSemestralResultadosDAO = new AuthSemestralResultadosDAO();
    $AuthSemestralResultadosVO = new AuthSemestralResultadosVO();
    $AuthSemestralDAO = new AuthSemestralDAO();
    $AuthSemestralVO = new AuthSemestralVO();
    $UsuarioDAO = new UsuarioDAO();
    $UsuarioVO = new UsuarioVO();

    $SqlBusqueda = "SELECT id FROM auth_semestral_resultados WHERE id_auth_semestral = " . $request->getAttribute("id_Bitacora") . " AND id_authuser  = " . $request->getAttribute("id_authuser");
    $idRg = utils\IConnection::execSql($SqlBusqueda);
    $StsAnterior = $request->getAttribute("Status") === "true" ? "false" : "true";
    $StsAuth = $request->getAttribute("Status") === "true" ? "active" : "inactive";

    $AuthSemestralVO = $AuthSemestralDAO->retrieve($request->getAttribute("id_Bitacora"));
    $UsuarioVO = $UsuarioDAO->retrieve($AuthSemestralVO->getId_authuser());
    $htmlAddG = $StsAuth === "inactive" ? "BAJA" : "ACTIVACION";
    BitacoraDAO::getInstance()->saveLog($UsuarioVO->getNombre(), "ADM", "$htmlAddG DE ID Usuario : " . $request->getAttribute("id_authuser"));

    $AuthSemestralResultadosVO->setId_auth_semestral($request->getAttribute("id_Bitacora"));
    $AuthSemestralResultadosVO->setId_authuser($request->getAttribute("id_authuser"));
    $AuthSemestralResultadosVO->setStatus_actual($request->getAttribute("Status"));
    $AuthSemestralResultadosVO->setStatus_anterior($StsAnterior);
    $AuthSemestralResultadosDAO->create($AuthSemestralResultadosVO);

    $Update = "UPDATE authuser SET status = '$StsAuth' WHERE id = " . $request->getAttribute("id_authuser");
    utils\IConnection::execSql($Update);
} else if ($request->getAttribute("Op") === "FormaDePago") {
    $display["Html"] = ComboboxFormaDePago::generateFP("Formadepago", "250px", $request->getAttribute("MetodoPago"), $request->getAttribute("Formapago"));
} elseif ($request->getAttribute("Op") === "ObtenDireccion") {
    $objectVO = new DireccionVO();
    $direccionDAO = new DireccionDAO();
    $objectVO = $direccionDAO->retrieve($request->getAttribute("idDireccion"), "id", " AND tabla_origen = '" . $request->getAttribute("Origen") . "'");
    $display["Id"] = $objectVO->getId();
    $display["Descripcion"] = $objectVO->getDescripcion();
    $display["Calle"] = $objectVO->getCalle();
    $display["NumExt"] = $objectVO->getNum_exterior();
    $display["NumInt"] = $objectVO->getNum_interior();
    $display["Colonia"] = $objectVO->getColonia();
    $display["Localidad"] = $objectVO->getLocalidad();
    $display["Municipio"] = $objectVO->getMunicipio();
    $display["Estado"] = $objectVO->getEstado();
    $display["CodigoPostal"] = $objectVO->getCodigo_postal();
} elseif ($request->getAttribute("Op") === "AgregarUbicacion") {
    /* Creacion de registro para agregar una dirección de un proveedor */
    $objectVO = new DireccionVO();
    $direccionDAO = new DireccionDAO();
    $objectVO->setDescripcion($request->getAttribute("Descripcion"));
    $objectVO->setCalle($request->getAttribute("Calle"));
    $objectVO->setNum_exterior($request->getAttribute("NumExt"));
    $objectVO->setNum_interior($request->getAttribute("NumInt"));
    $objectVO->setColonia($request->getAttribute("Colonia"));
    $objectVO->setLocalidad($request->getAttribute("Localidad"));
    $objectVO->setMunicipio($request->getAttribute("Municipio"));
    $objectVO->setEstado($request->getAttribute("Estado"));
    $objectVO->setCodigo_postal($request->getAttribute("CodigoPostal"));
    $objectVO->setTabla_origen($request->getAttribute("TablaOrigen"));
    $objectVO->setId_origen($request->getAttribute("IdProveedor"));
    $direccionDAO->create($objectVO);
} elseif ($request->getAttribute("Op") === "ActualizarUbicacion") {
    $objectVO = new DireccionVO();
    $direccionDAO = new DireccionDAO();
    $objectVO = $direccionDAO->retrieve($request->getAttribute("IdUbicacion"));
    $objectVO->setDescripcion($request->getAttribute("Descripcion"));
    $objectVO->setCalle($request->getAttribute("Calle"));
    $objectVO->setNum_exterior($request->getAttribute("NumExt"));
    $objectVO->setNum_interior($request->getAttribute("NumInt"));
    ...
    $objectVO->setEstado($request->getAttribute("Estado"));
    $objectVO->setCodigo_postal($request->getAttribute("CodigoPostal"));
    $objectVO->setId($request->getAttribute("IdUbicacion"));
    $direccionDAO->update($objectVO);
} elseif ($request->getAttribute("Op") === "EliminaUbicacion") {
    $Update = "UPDATE catalogo_direcciones SET id = - id WHERE id = " . $request->getAttribute("IdUbicacion");
    utils\IConnection::execSql($Update);
}
