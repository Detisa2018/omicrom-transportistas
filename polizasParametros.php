                    ...
                    success: function (response) {
                        ...
                        $.each(rs.cortes, function (index, value) {
                            ...
                            if (rs.cortes[index].statusctv === "Abierto") {
                                htmlContent += "<font color=\"red\">No es posible generar la Póliza del día " + fecha + ":<br/>";
                                htmlContent += "<font color=\"blue\">El corte " + rs.cortes[index].id + " " + rs.cortes[index].concepto + " " + rs.cortes[index].descripcion + " no ha sido cuadrado.</font></br>";
                                $("#dateStatus").html(htmlContent); 
                                cuadrado = false;
                            }
                        });
                        ...
                        if (rs.pagos.length === 0) {
                            htmlContent += "<font color=\"red\">No es posible generar la Póliza del día " + fecha + ":<br/>";
                            htmlContent += "<font color=\"blue\">No ha capturado los depósitos para la fecha seleccionada.</font><br/>";
                            $("#dateStatus").html(htmlContent); 
                            return false;
                        }
                        let cerrado = true;
                        $.each(rs.pagos, function (index, value) {
                            if (rs.pagos[index].status === "Abierta") {
                                htmlContent += "<font color=\"red\">No es posible generar la Póliza del día " + fecha + ":<br/>";
                                htmlContent += "<font color=\"blue\">Pago abierto: " + rs.pagos[index].fecha + " " + rs.pagos[index].concepto + " " + rs.pagos[index].nombre + ".</font></br>";
                                $("#dateStatus").html(htmlContent); 
                                cerrado = false;
                            }
                        });
