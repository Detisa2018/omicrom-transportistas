...
$Titulo = "Venta a Crédito del corte $Corte ";
$Msj = urldecode(utils\HTTPUtils::getRequest()->getAttribute("Msj"));

$cVarVal = 'report_ieps_green';

$greenBoxDAO = new V_CorporativoDAO();
$greenBoxVO = $greenBoxDAO->retrieve($cVarVal)->getValor();
...
$selectVentasCredito = "
        SELECT rm.id, rm.cliente, cli.nombre nombrec, cli.tipodepago, com.descripcion producto,  DATE_FORMAT(rm.fin_venta, '%d-%m-%Y %T' ) fin_venta,rm.descuento,
        rm.posicion, rm.volumen, ROUND(rm.pesos,2) pesos,  ROUND(rm.pagoreal,2) pagoreal, rm.uuid
        FROM com, cli, rm, man
        WHERE 1 = 1
        AND com.clavei = rm.producto 
        AND rm.cliente = cli.id 
        AND rm.posicion = man.posicion AND man.activo = 'Si'
        AND rm.corte = $Corte 
        AND cli.tipodepago IN ('Credito','Prepago','AutoConsumo','Cortesía') 
        ";
if (is_numeric($IslaPosicion)) {
    $selectVentasCredito .= " AND man.isla_pos = $IslaPosicion";
}
if ($Status !== "*") {
    if ($Status === "0") {
        $selectVentasCredito .= " AND ROUND(rm.pesos,2) = ROUND(rm.pagoreal,2)";
    } elseif ($Status === "1") {
        $selectVentasCredito .= " AND ROUND(rm.pesos,2) <> ROUND(rm.pagoreal,2)";
    }
}
...
$selectVentasAceiteCredito = "
        ...
        AND cli.tipodepago IN ('Credito','Prepago') 
        ";
if (is_numeric($IslaPosicion)) {
    $selectVentasAceiteCredito .= " AND man.isla_pos = $IslaPosicion";
}
$selectVentasAceiteCredito .= " 
        ORDER BY vt.clave";

$registros = utils\IConnection::getRowsFromQuery($selectVentasCredito);

$registrosA = utils\IConnection::getRowsFromQuery($selectVentasAceiteCredito);

$self = utils\HTTPUtils::getEnvironment()->getAttribute("PHP_SELF");
$returnLink = "movvtascre.php";
$ValidaPagoReal = "SELECT valor FROM variables_corporativo  WHERE llave = 'SinPagoReal'";
$Pr = utils\IConnection::execSql($ValidaPagoReal);
?>
...
<html lang="es" xml:lang="es">
            ...
            $(document).ready(function () {
                ...
                $("#autocomplete")
                        ...
                        .activeComboBox(
                                ...
                                "WHERE TRUE AND cli.tipodepago in ('Credito','Prepago','AutoConsumo','Cortesía') AND cli.activo = 'Si' ",
                                "nombre");
                $("#Corte").val("<?= $Corte ?>");
    ...
    <body>
                ...
                if ($ctVO->getStatusctv() === StatusCorte::ABIERTO) {
                    ...
                    <form name="form2" id="form3" method="post" action="">
                        <div class="row no-padding" id="InicialA">
                            ...
                            <div class="col-6"></div>
                            <div class="col-1"><select name="Status" id="Status" onchange="submit();"><option value="*">Todos</option><option value="0">Completos</option><option value="1">Modificados</option></select></div>
                        </div>
                    </form>
                    <form name="form3" id="form3" method="post" action="">
                        <div class="row no-padding InicialB">
                            <div class="col-2 align-right">Ticket: <span id="TicketSpan">0</span></div>      
                            <div class="col-1 align-right">Placas : </div>
                            <div class="col-1"><input type="text" name="Placas" maxlength="20"></div>
                            <div class="col-1 align-right">Kilometraje : </div>
                            <div class="col-1"><input type="number" name="Kilometraje" max="10000000"></div>
                            ...
                            if (!($Pr["valor"] == 1)) {
                                ...
                                ?>
                                <div class="col-2 align-right"><input type="hidden" name="Pagoreal" id="Pagoreal"></div>
                                <?php
                            }
                            ?>
                        </div>
                        <div class="row no-padding InicialB">
                            ...
                            <div class="col-1 ShowHideUnidad">Unidad : </div>
                            <div class="col-3 ShowHideUnidad">
                                <select name="Unidad" id="Unidad"></select>
                            </div>
                            <div class="col-2"><input type="button" name="Boton" value="Buscar Unidades" id="BuscaUnidades"></div>
        ...
        <div id="TablaDatos" style="min-height: 150px">
            <table aria-hidden="true">
                <tr>
                    ...
                    <td class="fondoNaranja">Group</td>
                </tr>
                ...
                foreach ($registros as $rg) {
                    ...
                    $addTitle = "";
                    if ($rg["descuento"] > 0) {
                        $addTitle = "Contiene un descuento del " . $rg["descuento"];
                    }
                    if ($rg["pesos"] <> $rg["pagoreal"] && abs($rg["pesos"] - $rg["pagoreal"]) > 0.5) {
                        $style = "background-color: #F7FF7C";
                    }
                    if (ROUND($rg["pesos"] - $rg["descuento"], 2) == ROUND($rg["pagoreal"], 2) && $rg["descuento"] > 0) {
                        $style = "background-color: #82E0AA";
                    }
                    ?>
                    <tr style="<?= $style ?>" title="Fecha del consumo: <?= $rg["fin_venta"] ?> <?= $addTitle ?>" >
                        ...
                        <?php if ($rg["tipodepago"] == 'Prepago') { ?>
                            <td><a style="" href="javascript:window.winieps('asigvale.php?criteria=ini&cli=<?= $rg["cliente"] ?>&idT=<?= $rg["id"] ?>&nomb=<?= $rg["nombrec"] ?>&importe=<?= $rg["pagoreal"] ?>')" ><?= $rg["tipodepago"] ?></a></td>
                        <?php } else { ?>
                            <td><?= $rg["tipodepago"] ?></td>
                        <?php } ?>
                        ...
                        <td style="text-align: center;">
                            <?php if ($ctVO->getStatusctv() === StatusCorte::ABIERTO) { ?>
                                <a class="textosCualli_i_n" href=javascript:borrarRegistro("<?= $self ?>","<?= $rg["id"] ?>","tipo=C&Tarjetas=0&returnLink=<?= $returnLink ?>&cId");><i class="icon fa fa-lg fa-trash" aria-hidden="true"></i></a>
                            <?php } ?>
                        </td>
                        <td><input type="checkbox" name="Desliga" data-idrg="<?= $rg["id"] ?>" data-idcliente="<?= $rg["cliente"] ?>" class="Desliga"></td>
                    </tr>
                    ...
                    if ($registros[$nRng + 1][$orden] !== $rg[$orden]) {
                        echo "<tr>
                        <td class='upTitles' colspan='1'>Transacciones</td></td>
                        <td class='upTitles' colspan='1'>" . number_format($cont, 0) . "</td>
                        <td class='upTitles' colspan='5'>Subtotal</td></td>
                        <td class='upTitles' colspan='2'>" . number_format($subV, 2) . "</td>
                        <td class='upTitles' colspan='2'>" . number_format($subDD, 2) . "</td>
                        <td class='upTitles' colspan='1'>" . number_format($subI, 2) . "</td>
                        <td class='upTitles'></td>
                        <td class='upTitles'><i class='fa-solid fa-user-minus'  data-idclienteg='" . $rg["cliente"] . "'></i></td>
                        </tr>";
                        $subI = $subV = $cont = 0;
                    }
        ...
        <div class='texto_tablas' align='center'><strong>Total venta a credito $ <?= number_format($nImp + $nImpAce, 2) ?></div>

        <?php echo $paginador->footer(false, null, false, false, 0, false); ?>

        <div class='texto_tablas'  align='right'>
            <?php if ($usuarioSesion->getLevel() >= 8 & $ctVO->getStatusctv() === StatusCorte::ABIERTO && $greenBoxVO == 1) { ?>
                <a href=javascript:winuni('repconcentradoc.php?Corte=<?= $Corte ?>');>
                    <div style="border: solid 0px gray;width: 15px;height: 15px;">
                        <div style="color: limegreen;width: 14px;height: 14px;padding-top: auto;"><i class="icon fa fa-lg fa-th-large" aria-hidden="true"></i></div>
                    </div>
                </a>
            <?php } ?>
        </div>        
        ...
        <script>
            $(document).ready(function () {
                $(".ShowHideUnidad").hide();
                $("#BuscaUnidades").click(function () {
                    $(".ShowHideUnidad").show();
                    let partes = $("#autocomplete").val().split(" ");
                    var idCliente = partes[0];
                    jQuery.ajax({
                        type: "POST",
                        url: "getByAjax.php",
                        dataType: "json",
                        cache: false,
                        data: {"Op": "ObtenUnidades", "IdCli": idCliente},
                        success: function (unidades) {
                            $("#Unidad").empty();
                            let nuevaOpcion = $("<option></option>").val("").text("SELECCIONE UNA UNIDAD");
                            $("#Unidad").append(nuevaOpcion);
                            $.each(unidades, function (key, value) {
                                nuevaOpcion = $("<option></option>").val(key).text(value);
                                $("#Unidad").append(nuevaOpcion);
        ...
        <script type="text/javascript" src="js/actionMov.js?var=<?= md5("js/actionMov.js") ?>"></script>
    </body>
</html>
