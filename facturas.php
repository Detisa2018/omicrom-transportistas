    ...
    <head>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $(".muestraEnvio").click(function () {
                    let valor = $(this).data('info');
                    let icon = $(this).data('pass');
                    console.log(valor);
                    let iconShow = "";
                    if (icon === "green") {
                        iconShow = "success";
                    } else if (icon === "graytext") {
                        iconShow = "error";
                        valor = "Correo no enviado";
                    } else {
                        iconShow = "error";
                    }
                    Swal.fire({
    ...
    <body onload="pop();">
        ...
        <div id="TablaDatos">
            <table class="paginador" aria-hidden="true">
                <?php
                if (empty($session->getSessionAttribute("returnLink"))) {
                    echo $paginador->headers(array("Editar", "Detalle", "Pdf", "Envio", "Xml"), array("Status", "Origen"));
                    while ($paginador->next()) {
                        $row = $paginador->getDataRow();
                        $SqlMailer = "SELECT mailer FROM facturas WHERE uuid = '" . $row["uuid"] . "'";
                        $Mailer = utils\IConnection::execSql($SqlMailer);
                            ...
                            if ($row['uuid'] !== FcDAO::SIN_TIMBRAR && !empty($row['uuid'])) {
                                if ($row['status'] != StatusFactura::CANCELADO) {
                                    if ($row['receptor'] !== FcDAO::RFC_GENERIC) {
                                        $colorEnvelop = strpos($Mailer['mailer'], "SUCCESS") !== false ? "green" : "red";
                                        $colorEnvelop = strlen($Mailer['mailer']) == 0 ? "graytext" : $colorEnvelop;
                                        $iconEnvelop = strlen($Mailer['mailer']) == 0 ? "fa-solid fa-circle-exclamation" : "icon fa fa-lg fa-envelope-o";
                                        $titleEnvelop = $Mailer['mailer'];
                                        ?>
                                        ...
                                        <td style="text-align: center;">
                                            <a style="color: <?= $colorEnvelop ?>;" href="#"><i class="<?= $iconEnvelop ?> muestraEnvio" data-pass="<?= $colorEnvelop ?>" data-info="<?= $titleEnvelop ?>" title="<?= $titleEnvelop ?>" aria-hidden="true"></i></a>
                                        </td>
                                    <?php } else {
                                        ?>
                                        <td style="text-align: center;">
        ...
        <?php
        $nLink = array("<i class=\"fa-regular fa-file-lines\"></i>Publico General" => "javascript:wingral('publicoGral.php?criteria=ini')");
