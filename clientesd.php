...
$paginador = new Paginador($Id,
        "unidades.codigo,unidades.id,IF(unidades.importe > 0, unidades.importe, unidades.litros) cantidad, IF(unidades.estado = 'a', 'Activo','Inactivo') status,unidades.periodo",
        "",
        "",
    ...
    <head>
        ...
        <script>
            $(document).ready(function () {
                $("#autocomplete").focus();
                $("#CCodigo").click(function () {
                    jQuery.ajax({
                        type: "POST",
                        url: "getByAjax.php",
                        dataType: "json",
                        cache: false,
                        data: {"Op": "CrearCodigoCliente", "Cliente": "<?= $clienteVO->getId() ?>"},
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    jQuery.ajax({
                                        ...
                                        dataType: "json",
                                        cache: false,
                                        data: {"Op": "CreaciónAsignacionCodigo", "Cliente": "<?= $clienteVO->getId() ?>", "CodigoNvo": codigo, "Usr": "<?= $usuarioSesion->getUsername() ?>"},
                                        success: function (data) {
                                            location.reload();
                                        }
                                    });
    ...
    <body>
        ...
        <div style="width: 98%;margin-left: auto;margin-right: auto;border: 2px solid gray;margin-bottom: 10px;padding: 3px 1px;">
            <table style="width: 98%;margin-left: auto;margin-right: auto;" class="texto_tablas" aria-hidden="true">
                <tr style="background-color: #E1E1E1;height: 20px;">
                    ...
                    <td style="text-align: center;font-size: 20px !important;"><a href="#" class="textosCualli" id="CCodigo">Creación de codigo</a></td>
                </tr>
            </table>
        </div>
        ...
        <div id="TablaDatos">
            <table class="paginador" aria-hidden="true">
                <?php
                echo $paginador->headers(array("Configurar", "Imprime"), array("Periodo", "Cantidad", "Estatus", " "));
                while ($paginador->next()) {
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;"><a href=javascript:winuni("impcodigo.php?busca=<?= $row['codigo'] ?>") title="Configurar unidad"><i class="fa-solid fa-barcode"></i></a></td>
