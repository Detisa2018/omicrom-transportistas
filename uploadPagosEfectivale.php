...
$BuscaUuid = "SELECT * FROM fc WHERE observaciones LIKE '%" . $_FILES["file"]["name"][0] . "%'";
error_log($BuscaUuid);
$Dt = $mysqli->query($BuscaUuid)->fetch_array();
error_log("VALORES  " . print_r($Dt, true));
if (!($Dt["id"] > 0)) {
    error_log("ENTRAMOS");
    if (move_uploaded_file($_FILES["file"]["tmp_name"][0], "/home/omicrom/xml/" . $_FILES["file"]["name"][0])) {
        ...
        $Comprobante = array();
        foreach ($carga_xml->xpath('//cfdi:Comprobante//cfdi:Complemento//ConsumoDeCombustibles:ConsumoDeCombustibles') as $comp) {
        ...
        error_log("Aqui contenemos comprobante total : " . $Comprobante['Total']);
        ...
        $PagoVO->setConcepto("Carga XML " . $_FILES["file"]["name"][0] . " | ");
        error_log("CReamos los pagos    " . print_r($PagoVO, true));
        if ($IdMc = $PagoDAO->create($PagoVO)) {
            error_log("Se crea el pago");
            ...
            $DescuentoDie = 0.00;
            $array = array();
//            foreach ($carga_xml->xpath('//cfdi:Comprobante//cfdi:Conceptos//cfdi:Concepto//cfdi:Impuestos//cfdi:Traslados//cfdi:Traslado') as $Traslado) {
//                $array[$e] = $Traslado["Importe"];
//                $e++;
//            }
            $e = 0;
            foreach ($carga_xml->xpath('//cfdi:Comprobante//cfdi:Complemento//ConsumoDeCombustibles:ConsumoDeCombustibles//ConsumoDeCombustibles:Conceptos//ConsumoDeCombustibles:ConceptoConsumoDeCombustibles//ConsumoDeCombustibles:Determinados//ConsumoDeCombustibles:Determinado') as $Traslado) {
                $array[$e] = $_REQUEST["MontoNeto"] == 1 ? $Traslado["importe"] : 0;
                $e++;
            }

            $e = 0;
            foreach ($carga_xml->xpath('//cfdi:Comprobante//cfdi:Complemento//ConsumoDeCombustibles:ConsumoDeCombustibles//ConsumoDeCombustibles:Conceptos//ConsumoDeCombustibles:ConceptoConsumoDeCombustibles') as $ConcComDeCom) {
                (double) $Impuesto = 0.00;

                $Impuesto = (double) $array[$e];
                if ($ConcComDeCom["tipoCombustible"] == 1 || $ConcComDeCom["tipoCombustible"] == 5) {
                    (double) $CnMg = (double) $ConcComDeCom['cantidad'];
                    (double) $ImpMg = (double) $ConcComDeCom['importe'] + $Impuesto;
                    ...
                    (double) $IpPm = (double) $ConcComDeCom['importe'] + $Impuesto;
                    ...
                    $ImportePrem += $IpPm;
                    $DescuentoPrem += $DsPm;
                } else if ($ConcComDeCom["tipoCombustible"] == 3) {
                    (double) $CnDs = (double) $ConcComDeCom['cantidad'];
                    (double) $IpDs = (double) $ConcComDeCom['importe'] + $Impuesto;
            ...
            error_log("CREATE DE FC " . print_r($FcVO, true));
            if ($Idfc = $FcDAO->create($FcVO)) {
                $Fc1VO = new FcVO();
                $Fc1VO = $FcDAO->retrieve($Idfc);
                $noEstacion = explode("E", $ciaVO->getNumestacion());
                $Observaciones = "Factura: " . $_FILES["file"]["name"][0] . " | " . $noEstacion[1] . "  " . $Comprobante['Folio'] . " " . $Comprobante['RfcEmisor'];
...
function TotalizaFactura($factura, $fcDAO) {
    ...
    $SqlCorreccionGeneral = "SELECT SUM(fcd.importe) total,((SUM(fcd.importe) - fc.ieps)/1.16)*0.16 iva,
            ...
            fc.ieps,fc.total,pagos.id 
            FROM fcd LEFT JOIN fc ON fc.id=fcd.id LEFT JOIN pagos ON fc.relacioncfdi=pagos.id where fc.id = " . $factura;
    ...
    $SqlCorreccionGeneral = "UPDATE fc SET importe = " . $rsC["importe"] . ",iva = " . $rsC["iva"] . ",total = " . $SumFcd . " WHERE id= " . $factura;
    utils\IConnection::execSql($SqlCorreccionGeneral);
}
