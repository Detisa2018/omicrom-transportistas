...
require "./services/ConsultasReportes.php";

error_log("Consulta a Ejecutar" .$cSql);
$result = $mysqli->query($cSql);
error_log("Rows: " . $result->num_rows);
$count = $result->field_count;
error_log("Fields: " . $count);
$registros = utils\IConnection::getRowsFromQuery($cSql, $mysqli);
error_log("Fields: " . $registros);
...
if ($request->getAttribute("Detallado") === "Si"):
    ...
    foreach ($registros as $datos) :
        ...
        foreach ($Filtro as $key => $value) :
            if ($datos[$value] !== $registros[$rows][$value]) :
                $column = 0X41;
                $row++;
                ...
                for ($i = 0; $i < $count; $i++) :
                    $ident = chr($column) . "" . $row;
                    if ($i == $value) {
                        $objPHPExcel->setActiveSheetIndex($sheet)->setCellValue($ident, $datos[$value]);
                    } else {
