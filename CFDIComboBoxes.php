...
class ComboboxAddenda {

    static function generate($comboID) {
        $mysqli = iconnect();
        $qry = $mysqli->query("SELECT id clave, nombre descripcion FROM addenda WHERE status = 'A'");

        $html = "<select  class='texto_tablas' name='$comboID' id='$comboID'><option value=''>ASIGNAR ADDENDA</option>";
        while ($rs = $qry->fetch_array()) {
            $html .= OPTION . $rs["clave"] . "'>" . $rs["clave"] | $rs["descripcion"] . CLOPTION;
        }
...
class ComboboxFormaDePago {
    ...
    static function generateFP($comboID, $width = "150px", $MetodoPago, $FormaPago) {
        $mysqli = iconnect();
        $HtmlAdd = $MetodoPago === "PPD" ? " AND clave = '99'" : " AND clave != '99'";
        $Sql = "SELECT clave, descripcion FROM cfdi33_c_fpago WHERE status = 1 $HtmlAdd";
        $qry = $mysqli->query($Sql);
        $html = "<select  class='texto_tablas' name='$comboID' id='$comboID' style='width: $width'><option value=''>SELECCIONE FORMA DE PAGO</option>";
        while ($rs = $qry->fetch_array()) {
            if ($FormaPago === $rs["clave"]) {
                $html2 .= "<option selected value='" . $rs["clave"] . "'>" . $rs["clave"] . " | " . $rs["descripcion"] . "</option>";
            } else {
                $html2 .= "<option value='" . $rs["clave"] . "'>" . $rs["clave"] . " | " . $rs["descripcion"] . "</option>";
            }
        }
        $html .= $html2 . "</select>";
        return $html;
