...
try {
    ...
    if (($query = $omiConn->query("SELECT valor FROM variables_corporativo WHERE llave = 'pol_tipomov_abono'")) && ($rs = $query->fetch_assoc())) {
        $abono = $rs["valor"];
    }
    $fabono = empty($abono) ? "1" : $abono;
            ...
            if ($qryData->num_rows>0) {
                while (($rsData = $qryData->fetch_assoc())) {
                    if ($rsData['NCC']!='') {
                        ...
                        if ((!empty($rsData['CargoAbono']) && $rsData['CargoAbono'] == "C")
                            || (!empty($rsData['TipoMovimiento']) && $rsData['TipoMovimiento'] == $fabono)) {
    ...
    if ($total<0) {
        $cuadreParameters["fecha"] = $queryParameters["fecha"];
